...
>
> **Amended during execution, round 7 (2026-08-02):** a further Codex review (PR feedback, round 7) found two defects. First, `commands/ticket.md` and `commands/finalize.md` cited `partial:epic-update` on any non-empty `SKIPPED` bucket, but `epic-update/SKILL.md` defines a concrete `SKIPPED` item as a **user decision** — the interactive follow-up gate offered File/Skip and the user chose Skip — which §6 already treats as routine interaction that must never be logged. The trigger narrowed to a non-empty `FAILED-TO-FILE` bucket, or either bucket carrying `epic-update`'s **`unknown`** sentinel (recorded when `REVIEW_REPORT` arrived absent and the invocation had no basis to assert either bucket) — that sentinel, not a concrete `SKIPPED`, is the quiet degradation this signature exists to catch. Second, `ticket-system/SKILL.md` documented `fetchTicket` collapsing an absent, unset, **or wrong-typed** `epicMarkerProperty` to the same `false` default, but only the absent case was ever recorded (`missing-property:epicMarkerProperty`, reused from `findEpics`) — a wrong-typed marker silently dropped an existing child ticket's epic context with nothing logged. `getEpicContext` now detects the wrong-typed case itself, at the same schema check where it already detects absence, and records a new `wrong-type:epicMarkerProperty` row (§7.4). `SKILL.md` and `references/signatures.md` are current for both points; the tables below reflect the pre-round-7 design.
>
> **Amended during execution, round 8 (2026-08-02):** a further Codex review (PR round 8) found round 4's fix above (mapping `getSelectOptions`/`addSelectOption`'s live-column-name argument back to its owning config key) was itself ambiguous: the config schema puts no uniqueness constraint on these property-name values, so two config keys can be configured to the same live column name (e.g. both `epicProperty` and `phaseProperty` set to `"Tags"`), and given only the live value there is no way to tell which key a call meant — one call could be recorded under the wrong signature with nothing surfacing the mislabeling. The fix removes the ambiguity at its source instead of trying to resolve it after the fact: both operations now take the config key as the argument the caller names directly (`getSelectOptions(configKey)`, `addSelectOption(configKey, optionName)`), and the adapter resolves that key to its live column name itself, purely to perform the Notion lookup — the citation direction is key → live name only, never reversed. `commands/create-task.md`'s call sites needed no change: they already invoke these operations as `getSelectOptions(<phaseProperty>)` / `addSelectOption(<phaseProperty>, "<phase>")`, naming the config key directly. The `staticProperties` fallback (no config key owns the argument) is unchanged in substance but reframed: it now covers a caller that genuinely has no key to name, not a lookup that failed to find one. `SKILL.md` (both `ticket-system` and `issue-log`) is current; the tables below reflect the pre-round-8 design.
>
> **Amended during execution, round 9 (2026-08-02):** round 8's claim, immediately above, that `commands/create-task.md`'s call sites "needed no change" was **wrong** and shipped a P1 contract break: the live file at the time still read `getSelectOptions(<epicProperty>)` / `getSelectOptions(<phaseProperty>)` each followed by a parenthetical — `(the configured name, default "Epic")` / `(the configured name, default "Phase")` — that explicitly instructed the caller to substitute the **resolved live column name**, the exact opposite of round 8's `configKey`-in contract; the matching `addSelectOption(<epicProperty>, "<proposed>")` / `addSelectOption(<phaseProperty>, "<phase>")` sites inherited the same instruction. Two agents in a row misread the `<epicProperty>`/`<phaseProperty>` angle-bracket notation as "fill in the configured value here" rather than "this is the literal key" — the notation itself was the trap, since `<proposed>`/`<phase>` elsewhere in the same file legitimately are fill-in-the-value placeholders. The fix drops the angle brackets from the config-key argument at every call site (`getSelectOptions(epicProperty)`, `getSelectOptions(phaseProperty)`, `addSelectOption(epicProperty, "<proposed>")`, `addSelectOption(phaseProperty, "<phase>")`) so a bare identifier always means "pass this key literally" and angle brackets are reserved for genuine value substitution; each site keeps the retired parenthetical's useful fact (the live column defaults to `"Epic"`/`"Phase"`) but rephrased so it reads as background, not as a substitution instruction. No operation signature, adapter behavior, or registry row changed — this was caller-side prose only, confined to `commands/create-task.md`. `commands/create-task.md` is current; the round-8 note above is superseded on this one point.

> **Amended during execution, round 10 (2026-08-02):** a further Codex review found that round 7's fix above only closed the `getEpicContext` path — `/notion-dev:finalize` never calls `getEpicContext` at all; it calls `fetchTicket` directly and then `epic-update`, so on that path `epicMarkerProperty`'s collapse to `false` was never recorded, whether the live cause was absence, wrong type, or a genuinely unchecked box. Detection moved to the one operation every path shares: `fetchTicket` itself now distinguishes the three causes when it reads `epicMarkerProperty` in step 4a and records `missing-property:epicMarkerProperty` / `wrong-type:epicMarkerProperty` for the first two — the returned `metadata.epicMarkerProperty` value stays the same collapsed `false` in all three cases, so no caller's control flow changes. `issue-log/SKILL.md`'s §6 exemption for `fetchTicket`'s absence-tolerant reads now names `epicMarkerProperty` as an explicit exception, with the reasoning stated inline so a future editor does not fold it back into the general rule. `getEpicContext`'s own recording of both signatures — added in round 1 (absence) and round 7 (wrong type) respectively — is retired: it still returns `null` for the same three causes, but the recording is `fetchTicket`'s alone, already triggered by the caller's own preceding `fetchTicket` call (e.g. `/notion-dev:ticket` Phase 1.1's `fetchTicket(currentTicketId)`). `SKILL.md` (both `ticket-system` and `issue-log`) and `references/signatures.md` are current; the tables and notes above reflect the pre-round-10 design.
...
> **Amended during execution, round 10 (2026-08-02):** the claim on the line above was incomplete: `/notion-dev:finalize` never calls `getEpicContext`, `findEpics`, or `createTicket` at all, so on that path nothing had recorded `epicMarkerProperty`'s collapse before this round — the gap a further Codex review surfaced. `epicMarkerProperty` is now a named exception to the `fetchTicket` row above: `fetchTicket` itself distinguishes and records `missing-property:epicMarkerProperty` / `wrong-type:epicMarkerProperty` whenever it reads the property, for any ticket, on every path — the one operation `/notion-dev:ticket`, `/notion-dev:finalize`, and any future caller all share. `getEpicContext` no longer records either signature; it relies on the caller's own preceding `fetchTicket` call. `issue-log/SKILL.md`'s current never-logged table states the exception and its reasoning inline; this table stays a historical snapshot of the pre-round-10 design.
...
>
> **Amended during execution, round 7 (2026-08-02):** two changes, one new row — see the round-7 note on §5 above for the full rationale. The `partial:epic-update` row's Condition narrowed from "a non-empty `SKIPPED` or `FAILED` bucket" to "a non-empty `FAILED-TO-FILE` bucket, or either bucket carrying the `unknown` sentinel" — a concrete `SKIPPED` alone must never trigger it. The registry also gained `wrong-type:epicMarkerProperty` (`degraded`, `ticket-system`, "present but not a Checkbox type; `getEpicContext` returns `null`, epic context unavailable for that call", `once/run`) — the live registry now holds 23 rows, not the 22 the table below shows. `references/signatures.md` is current; this table stays a historical snapshot.
>
> **Amended during execution, round 8 (2026-08-02):** no row count change (still 23) — see the round-8 note on §5 above for the full rationale. Round 4's fix, immediately below, mapped `getSelectOptions`/`addSelectOption`'s live-column-name argument back to its owning config key before citing it; that mapping was itself ambiguous whenever two config keys share one live column name, since a bare live value can't tell the reader which key it was. The operations' signatures changed instead: both now take the config key as the argument the caller names directly, and the adapter resolves key → live name only for the Notion lookup, never the reverse, so there is nothing left to map back. `references/signatures.md` and both `SKILL.md` files are current; this table (and the round-4 note below it) stay a historical snapshot of the superseded design.

> **Amended during execution, round 10 (2026-08-02):** no row count change (still 23) — see the round-10 note on §5 above for the full rationale. Both `missing-property:epicMarkerProperty` and `wrong-type:epicMarkerProperty`'s `Condition` text changed to name `fetchTicket` (step 4a) as the sole recorder for every ticket fetch, not only `findEpics`/`getEpicContext`; `getEpicContext` no longer records either signature itself. `references/signatures.md` is current; this table stays a historical snapshot of the pre-round-10 design.

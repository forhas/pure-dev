...
**Local mode has no PR, so its freeze is only the file, and the file is editable.** This is a genuine weakness, stated rather than papered over. It is bounded by the same property that makes local mode acceptable elsewhere: the squash commit's `Unmet:` trailers (§6.2) still record what was not met, so a weakened criterion produces a visible absence of trailers rather than a silent pass.
...
**Local mode** — one `Unmet:` trailer per not-met criterion on the squash commit, mirroring the `Deferred:` trailers the convergence change introduced:
...
- **Interactive** — stop and ask. The run has genuinely failed to establish whether the work is done, and that is a fact worth a human's attention rather than a default.

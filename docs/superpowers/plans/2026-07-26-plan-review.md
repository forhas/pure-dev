...
```
Plan-review metrics come from Phase 2b step 3's output block (`ROUNDS`, `FINDINGS`, `ACCEPTED`, `DECLINED`, and `UNRESOLVED-CRITICAL` + `UNRESOLVED-REQUIRED` summed); all five are `null` wherever there is no review signal to record — the `feature-dev` path, which has no plan to review, and a `degraded` review, where the reviewer never ran. On a degraded review write `null`, **not** the zeros its output block carries: `0` findings would be indistinguishable from a review that ran and found nothing, and that is exactly the distinction this ledger exists to preserve.
```
...
It dispatches a fresh reviewer against the plan **and the codebase**, triages the findings, revises `PLAN.md`, and returns a `PLAN-REVIEW:` output block. Record the whole output block as `PLAN_REVIEW_REPORT` — `PLAN-REVIEW`, `ROUNDS`, `FINDINGS`, `ACCEPTED`, `DECLINED`, `UNRESOLVED-CRITICAL`, `UNRESOLVED-REQUIRED`, `PLAN-CHANGED`, `NOT-IN-SCOPE`, `DECLINED-WITH-REASONING`, and `UNRESOLVED` — for the ledger outcome and the ticket's `## Implementation` section (6.5). The revision preserves every `- [ ]` checkbox, so Phase 1.2's resume detection is unaffected.

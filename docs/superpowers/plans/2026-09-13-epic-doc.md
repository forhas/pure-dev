...
mut 'equals the remote base' "$ED" 's/rev-parse origin\/<baseRefName>/rev-parse origin\/main/'
...
mut 'ticket.md 1.1 reads the brief' "$T" 's/operation `read(metadata\.parentTaskProperty, <id>)`/operation `read()`/'
mut 'no longer calls getEpicContext'   "$T" '0,/epic-doc/s/epic-doc/getEpicContext( epic-doc/'
mut 'ticket.md records after the draft' "$T" 's/^\*\*Epic doc — record the resolution\.\*\* Runs after the draft report is composed and before/**Epic doc — record the resolution.** Runs after/'
mut 'Phase 10 order'                   "$T" 's/operation `record(<id>)`/operation `record()`/'
mut 'ticket.md cites' "$T" 's/record `partial:epic-doc` per/record `partial:epic-docs` per/'
mut 'ticket.md reports the epic doc line' "$T" 's/^- \*\*Epic doc\*\*/- **Epic brief**/'
...
mut 'finalize.md records after the draft' "$F" 's/^\*\*Epic doc — record the resolution\.\*\* Runs after the draft report is composed and before/**Epic doc — record the resolution.** Runs after/'
mut 'Phase 5 order'                    "$F" 's/operation `record(<id>)`/operation `record()`/'
mut 'finalize.md cites' "$F" 's/record `partial:epic-doc` per/record `partial:epic-docs` per/'
...
mut 'next-task reads the brief' "$NT" 's/operation `read(<epic-id>)`/operation `read()`/'
...
Save it as `mutate-epic-doc.sh` in your scratchpad directory (never in the repo) and run: `bash <scratchpad dir>/mutate-epic-doc.sh`

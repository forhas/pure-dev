...
4. **Completeness check** (local mode): local mode never invokes `quick-dev:review-and-merge`, so nothing else runs the Completeness gate for it — this step is that gate's local-mode equivalent, not a duplicate of it. Skip cleanly when `$REPO_ROOT/.claude/quick-dev/criteria-<SLUG>.md` does not exist (a resumed run that skipped 2a): report `CRITERIA-TOTAL: 0`, reporting zero criteria rather than "all met" — an absent file is not evidence of a done feature. Otherwise, dispatch a fresh `general-purpose` reviewer agent (synchronous) with the criteria file, the branch diff (`git diff "$BASE"...HEAD`), and — when step 2 or 3 applied fixes — the test output that re-run already produced. Local mode has no PR body to pass; say so in the prompt rather than inventing one. Instruct the agent to apply `../review-and-merge/SKILL.md`'s "### The completeness verifier" subsection exactly as written — the same three charges (per-criterion verdict with citation, unsupported completeness claims, untriaged caveats), the same anti-circularity rule, the same output block — rather than restating that contract here.

   **The gate resolves every citation, not the agent** — exactly as `review-and-merge` does: run command citations, match test citations against the tests step 1/3's loop actually ran, content-match code citations against the diff by content, never by line number. A citation that fails to resolve demotes its criterion to `unverified`, never `not-met`. On agent failure or malformed output, retry once; if it fails again, treat every criterion as `unverified` rather than guessing.

   Fold every `not-met` criterion, every `unverified` criterion, every unsupported claim, and every untriaged caveat into step 5's merge gate as `absorb` / `file` / `drop` items — completeness findings fold into this gate exactly like review findings; do not build a second gate. Record the resulting keyed block as `COMPLETENESS_REPORT` for Phase 6.

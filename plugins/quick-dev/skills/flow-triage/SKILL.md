---
name: flow-triage
description: This skill should be used when the user runs "/flow-triage" or "/quick-dev:flow-triage", asks "which flow fits this task", "triage this feature", "should this use feature-dev or superpowers", or when an orchestrating skill needs to choose a build flow for a feature description. Recommends feature-dev or superpowers via a codebase-aware scorecard and a per-repo outcome ledger.
argument-hint: "[--auto] [--advise-only] [--forced-flow=<flow>] [--ledger-root=<path>] [--run-id=<id>] <feature description>"
---
...
- `--ledger-root=<path>` — directory containing `.claude/quick-dev/`. Default: `git rev-parse --show-toplevel`. Callers running inside a worktree MUST pass the primary checkout's root explicitly.

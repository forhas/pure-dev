...
- Create the database via `mcp__notion__notion-create-database` with this schema:
  ...
  `Epic`, `Phase`, and `Step` are the structural-mission properties (`/notion-dev:create-task` mission path); without them a fresh install silently loses mission grouping and order — the same properties 3a-ii actively detects on existing DBs. **Do not add a `Depends on` relation here.** Blocking order is not stored in a relation: a single self-referential Notion relation is symmetric and cannot express direction, so `setDependencies` renders it into the ticket body as a `## Blocked by` section instead. A `Depends on` column created here would never be read or written by anything. `Parent task` carries a creation-order caveat: if the create API cannot declare a self-referential relation before the database's own ID exists, add it immediately afterward via `mcp__notion__notion-update-data-source` pointing at the new database. `Creation Date` is a plain Date property — the plugin writes the timestamp itself rather than using a `Created time` property, so the value stays editable and backfillable. `Is Epic` is a plain Checkbox — unlike the self-referential relation above, it carries no creation-order caveat and can be declared directly in the initial `mcp__notion__notion-create-database` call.
...
- **Detect structural-mission properties** — probe the live schema for the five optional properties used by multi-task missions. Detection is silent when a property resolves unambiguously; it prompts via `AskUserQuestion` only when it can't (an ambiguous candidate, or none found). For each, record an override in config only when the resolved live name differs from the default:
  ...
  - **No dependency relation is resolved.** `parentTaskProperty` is the only relation slot init binds. Blocking order between tickets is not stored in a relation at all — `setDependencies` renders it into the ticket body as a `## Blocked by` section — because a single self-referential Notion relation is symmetric and cannot express direction. Never add a `dependsOnProperty` slot back: a live `Depends on` column may well exist on the user's DB, but binding it would only reintroduce the symmetric writes this design exists to avoid. Leave any such column alone — do not bind it, warn about it, or offer to delete it; its contents may predate the plugin.

...
- Create the database via `mcp__notion__notion-create-database` with this schema:
  ...
  `Epic`, `Phase`, and `Step` are the structural-mission properties (`/notion-dev:create-task` mission path); without them a fresh install silently loses mission grouping and order — the same properties 3a-ii actively detects on existing DBs. **Do not add a `Depends on` relation here.** Blocking order is not stored in a relation — `setDependencies` renders it into the ticket body as a `## Blocked by` section — because the adapter cannot tell a one-way relation from a two-way one through the MCP schema surface (canonical statement in `skills/ticket-system/SKILL.md` → `setDependencies`; self-relations are **not** symmetric). A `Depends on` column created here would never be read or written by anything. `Parent task` carries a creation-order caveat: if the create API cannot declare a self-referential relation before the database's own ID exists, add it immediately afterward via `mcp__notion__notion-update-data-source` pointing at the new database. `Creation Date` is a plain Date property — the plugin writes the timestamp itself rather than using a `Created time` property, so the value stays editable and backfillable. `Is Epic` is a plain Checkbox — unlike the self-referential relation above, it carries no creation-order caveat and can be declared directly in the initial `mcp__notion__notion-create-database` call.
...
- **Detect structural-mission properties** — probe the live schema for the five optional properties used by multi-task missions. Detection is silent when a property resolves unambiguously; it prompts via `AskUserQuestion` only when it can't (an ambiguous candidate, or none found). For each, record an override in config only when the resolved live name differs from the default:
  ...
  - `parentTaskProperty` (default `"Parent task"`) — **the only relation slot init resolves.** **In reconfigure mode, apply the general reconfigure rule ("Re-configuration behavior" below) before any name-based scan**: take the existing config's effective `parentTaskProperty` (the written override if present, else the default `"Parent task"`) and check whether it still resolves live to a self-referential `relation` property of that exact name. If it does, bind it and skip the name-based scan below entirely — this is what stops a reconfigure from silently repointing an already-bound custom relation (e.g. `Epic parent`) onto a same-DB, default-named `Parent task` column that exists for an unrelated reason (such as Notion's native Sub-items). Only fall back to the scan below when the previously configured property no longer resolves live (renamed or deleted), or on a fresh run with no existing config to prefer. Otherwise resolve it in these steps:
    ...
    **(d) Whether to record `PARENT_NAME_TAKEN` depends on what got bound** — the same three-way split the marker slot below states in full, for the same reason (the signature's subject is the config key):
    ...
    - **Nothing bound and `PARENT_NAME_TAKEN` set** → **record `wrong-type:parentTaskProperty`** here, at detection. The key is omitted when unbound, so the effective property is the mistyped `Parent task`.
    - **Nothing bound and `PARENT_NAME_TAKEN` unset** → write nothing; a declined gate on a DB with no such column is routine.
  ...
  - **No dependency relation is resolved.** `parentTaskProperty` is the only relation slot init binds. Blocking order between tickets is not stored in a relation at all — `setDependencies` renders it into the ticket body as a `## Blocked by` section — for the reason stated once in `skills/ticket-system/SKILL.md` → `setDependencies`: the adapter cannot tell a one-way relation from a two-way one, so it cannot predict whether binding a given column will also produce reverse edges in a companion column it does not control. **This is a limit of the available API surface, not a property of Notion relations** — self-referential relations *are* directional, and an earlier version of this file wrongly claimed otherwise. Do not restore a `dependsOnProperty` slot on the strength of that corrected fact alone: what would justify restoring it is a reliable way to prove a candidate column is one-way, which the MCP schema surface does not currently provide. Until then, leave any live `Depends on` column alone — do not bind it, warn about it, or offer to delete it; its contents may predate the plugin.

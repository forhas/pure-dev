...
- **Detect structural-mission properties** — probe the live schema for the five optional properties used by multi-task missions. Detection is silent when a property resolves unambiguously; it prompts via `AskUserQuestion` only when it can't (an ambiguous candidate, or none found). For each, record an override in config only when the resolved live name differs from the default:
  ...
  - **No dependency relation is resolved.** `parentTaskProperty` is the only relation slot init binds. Blocking order between tickets is not stored in a relation at all — `setDependencies` renders it into the ticket body as a `## Blocked by` section — for the reason stated once in `skills/ticket-system/references/create-ops.md` → `setDependencies`: binding a relation column is a design judgment this plugin declines, for the three reasons the canonical statement gives. **This is not a property of Notion relations** — self-referential relations *are* directional, and an earlier version of this file wrongly claimed otherwise.

    Three corrected facts, each of which has already been stated wrongly here at least once, so state them plainly. Relations **are** directional. Subtype **is** detectable — `propertyUrl` marks two-way halves. And a `propertyUrl`-**absent** column is conclusively `single_property`, so writing to it is **provably safe**; a conservative guard binding only those columns would be sound, and this file must not imply otherwise.

    Companion state matters **only** on the `propertyUrl`-**present** branch, where a live dual half and an orphaned one are indistinguishable. Requiring it universally — as an earlier version of this paragraph did — reinstates the impossibility framing the rest of this change exists to remove, and would steer future init work away from support that is genuinely available.

    The slot stays unbound anyway, as the **design judgment** the canonical statement sets out with its three reasons — chiefly that binding it would make dependency order depend on a schema accident, when the body section already covers every database. Not because it cannot be done. If that tradeoff is ever revisited, the safe branch is well-defined and needs no new capability. What must **not** happen is reopening the `propertyUrl`-present branch on subtype detection alone.

    Either way, leave any live `Depends on` column alone — do not bind it, warn about it, or offer to delete it; its contents may predate the plugin.
...
- Similarly, only write `typeMap` / `statusMap` when one or more entries differ from the defaults (see `skills/ticket-system/references/config.md`).

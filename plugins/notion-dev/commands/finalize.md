...
5. Remove the worktrees parent directory if now empty: `rmdir <worktrees-parent>` — the parent of `<worktree-path>`, named explicitly so the target is checkable rather than implied. `rmdir`, never `rm -rf`: it refuses on a non-empty directory, which is the whole safety property here.

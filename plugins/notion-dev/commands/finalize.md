...
4. Resolve the worktree path as `$(dirname "$REPO_ROOT")/<repo-name>-worktrees/<prefix>`, with `<repo-name>` = `basename "$REPO_ROOT"` and `<prefix>` from the config's `worktree.prefix` template using that numeric `<id>` — the same layout `/notion-dev:ticket` Phase 1.2 creates and `quick-dev:develop` uses, and the layout step 5's `rmdir` depends on. If it's absent **and the PR is open**, recreate it (on the `MERGED` recovery path, never recreate — per step 2 — and when the worktree is absent there, skip the rest of this step including the `cd`; run Phases 3–4 from `$REPO_ROOT`):
   ```
   git fetch origin
   ```
   If a local branch `<headRefName>` exists: verify it matches the PR head (fast-forward it if behind), then `git worktree add <worktree-path> <headRefName>`. If no local branch exists, create the worktree detached and let `gh` resolve the head:

   ```bash
   git worktree add --detach <worktree-path>
   git -C <worktree-path> ... # then, from inside it:
   gh pr checkout <pr>
   ```

   **Not `git worktree add <worktree-path> -b <headRefName> origin/<headRefName>`**, which was what this step said and which cannot work on a fork PR — the head branch lives in the fork, so `origin/<headRefName>` is not a valid object and the command dies with `fatal: Not a valid object name`. That is exactly the case step 3 below goes out of its way to handle, and the asymmetry between the two steps was the defect: verified against a live fork PR. `gh pr checkout` fetches `refs/pull/<n>/head`, creates the local branch, and — this is the part a hand-rolled fetch would miss — sets `branch.<headRefName>.remote` to the **head repository**, so the review loop's fix commits push back to the fork rather than failing or landing on `origin`. It is equally correct for a same-repo PR, so there is one path, not two.
...
**Best-effort is not silent.** After the write, confirm **this run's** write landed — not merely that a file of that name is present. The path is deterministic per ticket and both entry points are re-runnable, so a stale `review-report-<KEY>-<id>.md` from an earlier run of the same ticket satisfies an existence test while this run's write failed: the failure is then neither logged nor reported, and the recovery path later consumes another run's review and completeness dispositions as if they were this one's. Confirm the write's own status, or read the file back and match it against the content just written. If it does not, record `unexpected:review-report-not-persisted` per `notion-dev:issue-log` and say so in Phase 5 — the run still continues. A persist that never happened and one that happened are otherwise indistinguishable afterwards, and Phase 1 step 2's recovery path will simply find nothing when it is needed most.

---

## Phase 3 — Record

**Resolve `epic-update`'s filing gate before the lock is taken.** In interactive mode that skill puts every `FILED` item to the user with `AskUserQuestion` — File as ticket, or Drop with a rationale — and a person deciding that can outlast the lock's 60-minute stale threshold, after which another run breaks a lock this one still believes it holds and both write at once. So ask now, holding nothing: walk `REVIEW_REPORT`'s `FILED` list, take each File/Drop answer, and pass the result into the invocation below as `FILING_DECISIONS`. A non-interactive run files everything and has nothing to ask. This is the rule `/notion-dev:new-info` and `/notion-dev:knowledge curate` already follow — no interactive gate is ever held under the primary lock, which is what the stale rule assumes.

**The `record` section begins here and ends after Phase 5's `record`.** `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/knowledge.py" lock take --run <run id> --section record --wait 3600` — `<run id>` is `finalize <pr>`. Exit 1 → stop per "Failure and stop conditions" with `CAUSE: primary lock held by <run> (<section>) since <time>`: cleanup cannot be skipped, so the run does not proceed without the lock. A `stale:` line → record `lock-stale:primary` and name the old owner in the report. Everything from here to the end of Phase 5's epic-doc step — `epic-update`'s rewrite of the Notion epic page, cleanup's checkout and pull of the primary, the post-merge hooks, and `record` — runs with the lock held. The section is bounded by the hour the lock's stale rule allows; a post-merge hook that cannot finish inside it must not be configured.
...
Invoke the `notion-dev:epic-update` skill via the Skill tool with args `<id>`, plus `--non-interactive` when set. Pass `REVIEW_REPORT` (Phase 2, or — on the `MERGED` recovery path — the persisted-file/reconstructed-history recovery in Phase 1 step 2, absent when neither yielded anything usable), `$REPO_ROOT` and `LOCK_HELD` as context.

**Only the `FILED` list** from `REVIEW_REPORT` is passed to `epic-update`. `ABSORBED` items are already merged, `DROPPED` items are already decided, and `BLOCKED` items cannot be worked by anyone until their named external cause changes; filing any of the three would recreate the non-convergence this split exists to stop. A `BLOCKED` item filed as a ticket is strictly worse than a forgotten one — it is a queue entry no future run can close. Carry it to Phase 5 instead, where it becomes a `blocked:` line in the closeout.
...
- **Blocked** — items from `REVIEW_REPORT`'s `BLOCKED` list, each with its external cause and what would unblock it. Omit the field when the list is empty. These carry no ticket ID and must never be given one: a `blocked` item is real work nobody can start, not deferred work somebody can.
...
1. If `<worktree-path>` is a registered worktree (`git worktree list` contains it — on the post-merge recovery path Phase 1 deliberately does not recreate a missing one, and a prior interrupted run may have already removed it), confirm it is the worktree resolved/created in Phase 1, then `git worktree remove <worktree-path>`. If it fails, retry with `git worktree remove --force <worktree-path>` — for untracked leftovers (e.g. build artifacts) or for `fatal: working trees containing submodules cannot be moved or removed`, which is deterministic in any repo that vendors dependencies as submodules rather than an anomaly (see Phase 9 step 1 of `commands/ticket.md`). Then `git worktree prune`. Absent → skip; only bookkeeping remains.
...
- Epic outcome, when the ticket had one: the epic's ID and URL, follow-ups absorbed, filed (with their IDs), and dropped, whether the epic closed, and **packets written vs. items filed** (3.2's assertion) whenever those two disagree. Omit the line entirely when the ticket had no epic.
...
- **Blocked items** — every entry in `REVIEW_REPORT`'s `BLOCKED` list, each as `blocked: <item> — <external cause>; unblocked by <what>`, which is the closeout's own line format so the two records cannot drift. These are **not** tickets and must never be reported as though they were: no ID, no URL, no "filed". State the count alongside `FILED` so the two are legible against each other. Say nothing when the list is empty.

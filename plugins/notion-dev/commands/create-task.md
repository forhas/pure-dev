...
2. `findEpics()` returned **`null`** (the DB's `epicMarkerProperty` is unusable — absent, or present but not a Checkbox; epics cannot be identified at all either way) → `EPIC_ID = undefined`, `EPIC_TO_CREATE` stays unset, and record **`EPIC_UNAVAILABLE_CAUSE = marker`** — carried through to Phase 4's mission report, which must name the property that actually failed rather than guess. Continue with Epic-select tagging only; do not prompt. (This is the "no epic containers on the DB" degradation path; Phase 3.2's deferred-creation step checks for it via `EPIC_TO_CREATE` being unset, so it never fires here either.) Neither branch writes to the issue log here. The `null` case is recorded once by `notion-dev:ticket-system` — as `missing-property:epicMarkerProperty` when the property is absent, or `wrong-type:epicMarkerProperty` when it exists but isn't a Checkbox; the `[]` case is routine and is never logged at all. See `notion-dev:issue-log`.
...
1. Invoke `notion-dev:ticket-system` operation `findEpics()`. **`null`** (the DB's `epicMarkerProperty` is unusable — absent, or present but not a Checkbox; epics cannot be identified at all either way) → skip silently, no prompt: the unavailable-degrade case. **`[]`** (`epicMarkerProperty` is usable but no page has it set to `true` yet) → also skip silently, no prompt, but for a different reason: there is simply nothing to attach to. Both are silent, but they are different states — do not conflate them in any future change here. Neither branch writes to the issue log here. The `null` case is recorded once by `notion-dev:ticket-system` — as `missing-property:epicMarkerProperty` when the property is absent, or `wrong-type:epicMarkerProperty` when it exists but isn't a Checkbox; the `[]` case is routine and is never logged at all. See `notion-dev:issue-log`.
...
- `EPIC_ID` undefined and `EPIC_PROPERTY_ABSENT` is **not** set (`epicProperty` exists, so the Epic select write succeeded; what failed is epic *containment*, because either `parentTaskProperty` or `epicMarkerProperty` was **unusable** — absent, or present but of the wrong type, which degrade identically — per Phase 2.5.2 and Phase 3.2's epic-creation step. Which of the two failed is known only in the `findEpics()` case; see `EPIC_UNAVAILABLE_CAUSE` below, which is the only thing the report may branch on. Never default to naming `Parent task` — that was a real defect here, reporting the wrong missing property in exactly the summary whose job is to say which happened.) → the Epic select write still succeeded on every task; only the container page is missing. Print: `Mission tagged with Epic select "<epic name>" on every task — no container page (<reason>).` Derive `<reason>` from **`EPIC_UNAVAILABLE_CAUSE`**, never from a hardcoded property name:
  ...
  - `marker` (Phase 2.5.2's `findEpics()` returned `null`) → `this DB has no usable "<epicMarkerProperty configured name>" checkbox, so epic containers cannot be identified` — substitute the configured name, exactly as the sibling branch does for `epicProperty`; the marker is configurable and `/notion-dev:init` writes an override whenever the bound checkbox is not named `Is Epic`, so the literal string would be a false claim about a column that does not exist.
  - `container-create` (Phase 3.2's `createEpic` returned `null`) → `epic containers are unavailable on this DB`. **Do not name a specific property here.** `createEpic` returns a bare `null` when *either* `parentTaskProperty` or `epicMarkerProperty` is unusable and does not report which, so any more specific wording would be a guess — and the marker can go from usable to unusable between 2.5.2 and 3.2, so "it must be the relation" is not a safe inference either. An unnamed-but-true reason is correct; a named-but-possibly-wrong one is the defect this rule exists to prevent.

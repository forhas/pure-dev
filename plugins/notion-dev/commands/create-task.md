...
2. `findEpics()` returned **`null`** (the DB lacks `epicProperty` or `parentTaskProperty` — epic containers are unavailable) → `EPIC_ID = undefined`, `EPIC_TO_CREATE` stays unset. Continue with Epic-select tagging only; do not prompt. (This is the "no epic containers on the DB" degradation path; Phase 3.2's deferred-creation step checks for it via `EPIC_TO_CREATE` being unset, so it never fires here either.)
3. Otherwise `findEpics()` returned an array (possibly `[]`, meaning the properties exist but no epic containers exist yet — a childless Epic-tagged page does not count, see `findEpics()` in `skills/ticket-system/SKILL.md`). A returned epic whose `name` matches the reconciled Epic value (case-insensitive) → reuse it. Record its **`id`** (the logical ticket id, e.g. `67` — *not* `pageId`) as `EPIC_ID`; `EPIC_TO_CREATE` stays unset.
...
1. Invoke `notion-dev:ticket-system` operation `findEpics()`. **`null`** (the DB lacks `epicProperty` or `parentTaskProperty` — epic containers are unavailable) → skip silently, no prompt: the unavailable-degrade case. **`[]`** (both properties exist but no epic containers exist yet — a childless Epic-tagged page is never returned as one) → also skip silently, no prompt, but for a different reason: there is simply nothing to attach to. Both are silent, but they are different states — do not conflate them in any future change here.

...
Then, unless this epic's `note --apply` outcome above was `failed`, invoke the `notion-dev:knowledge` skill, operation `capture --fact <fact> <epic-id>`, passing `KNOWLEDGE_CONTEXT`, `REPO_ROOT`, and `<epicBranch>` (under `--pr`, `--branch <noteBranch>`). Record its `KNOWLEDGE:` line for this epic's report; `failed` records `partial:knowledge-capture` and does not stop the loop — the next epic in scope still runs regardless of this outcome. This runs for both the `COMMIT: <sha>` and `COMMIT: none` cases above: the fact may still be worth capturing into the bundle even when the brief itself did not change.
...
2. Every apply, capture, and any bootstrap carries `--branch <noteBranch>`: `epic-doc` and `notion-dev:knowledge` then assert HEAD's branch is `<noteBranch>` and that `git merge-base --is-ancestor origin/<epicBranch> HEAD` exits 0 — the branch was cut from the epic branch and still contains it — instead of the primary-on-`<epicBranch>` and remote-equality lines, and commit **without pushing**.

...
Then, under `--pr` on a later epic: `git -C $REPO_ROOT fetch origin <epicBranch>`, and when `git -C $REPO_ROOT merge-base --is-ancestor origin/<epicBranch> HEAD` fails — another writer advanced the epic branch since the note branch was cut — the note branch is rebased onto it, next.

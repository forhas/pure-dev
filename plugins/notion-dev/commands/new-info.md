...
**The first epic needs the same freshness check, by a different route.** It never rebases — under `--pr` step 1 cuts `<noteBranch>` from `origin/<epicBranch>` as it is *now*, and the direct path's `pull --ff-only` fast-forwards onto it — so the ancestry check has nothing to fail on while the proposal can still have been computed against an older brief, and applying it overwrites a concurrent writer's update. Record the sha `read` resolved the brief at; on the first Apply, when `origin/<epicBranch>` no longer names that sha, re-derive exactly as above — fresh `EPIC_CONTEXT`, a fresh live child lookup, Propose re-run, `### Gate` re-entered only on a changed `DIFF` — before applying.

...
- `<epicBranch>` = `git.prTargetBranch`, falling back to `git.baseBranch` — the branch the brief lives on, per `notion-dev:epic-doc`. The primary is put on `<epicBranch>` below, right after the `apply` section's lock is taken, never here — checking out before the lock would let a second run's checkout race this one's.
...
Then, unless `--pr`: `git -C $REPO_ROOT checkout <epicBranch> && git -C $REPO_ROOT pull --ff-only origin <epicBranch>` (a `--ff-only` failure → release the lock and stop with the diverged-base report). Under `--pr`, `` ## `--pr` `` step 1's `git -C $REPO_ROOT checkout -b <noteBranch> origin/<epicBranch>` runs here instead.

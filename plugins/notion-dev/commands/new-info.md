...
- `<epicBranch>` = `git.prTargetBranch`, falling back to `git.baseBranch` — the branch the brief lives on, per `notion-dev:epic-doc`. The primary is put on `<epicBranch>` below, right after the `apply` section's lock is taken on the first epic (a later epic's re-take does not repeat the checkout), never here — checking out before the lock would let a second run's checkout race this one's.
- `<run id>` = `new-info-$(date -u +%Y%m%dT%H%M%SZ)-<4 hex>` (e.g. `new-info-20260915T101500Z-a3f9`; the hex from `head -c2 /dev/urandom | od -An -tx1 | tr -dc '0-9a-f'`), generated once at the start of this command and carried through every `lock take` and `lock release` of this run — so a second concurrent invocation is a different holder, while this run's own re-takes stay re-entrant.
...
**The `apply` section begins here, at the start of each epic's Apply.** `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/knowledge.py" lock take --run <run id> --section apply --wait 600` (`<run id>` from the preconditions; the take is re-entrant, so a later epic re-taking it under the same run id costs nothing; exit 1 → report this and every remaining epic as `skipped — primary lock held by <run> (<section>) since <time>`, record `lock-timeout:primary`, and go to the report). Every `record --bootstrap`, `note --apply` and `capture --fact` below receives `LOCK_HELD`.

Then, unless `--pr`: `git -C $REPO_ROOT checkout <epicBranch> && git -C $REPO_ROOT pull --ff-only origin <epicBranch>` (a `--ff-only` failure → release the lock and stop with the diverged-base report) — this runs after the take on the first epic. Under `--pr`, `` ## `--pr` `` step 1's `git -C $REPO_ROOT checkout -b <noteBranch> origin/<epicBranch>` runs here instead, likewise only on the first epic.
...
When that check fails: `git -C $REPO_ROOT rebase origin/<epicBranch>`: the note commits are local and unpushed, so nothing published is rewritten and no merge commit is manufactured. A rebase that succeeds means the brief this epic's proposal was computed against has moved, so re-derive before applying — the same re-derivation, for the same reason, that the `BOOTSTRAP: true` path below already performs: take `EPIC_CONTEXT` from `git show HEAD:<brief path>` **and re-run the live child lookup** — do not keep the `CHILDREN` already in hand, which is right for the `BOOTSTRAP: true` path below only because the brief moved there by *this* run's own commit, with Notion unmoved. A rebase means another writer landed changes that may include a child's status, title or dependencies, and `epic-doc` takes `CHILDREN` as the live input it derives `## Next` from, so reusing the pre-rebase collection can restore a resolved child, omit a new one, or render stale ordering over the very update that forced the rebase. Then re-run the Propose step alone, and re-enter `### Gate` only when the new `DIFF` differs from the accepted one. Applying the pre-rebase proposal would write a brief derived from content another writer has already landed. A conflicting rebase runs `git -C $REPO_ROOT rebase --abort`, releases the lock, and stops: this epic and every later one are reported `skipped — note branch could not be rebased onto <epicBranch>: <git's message>`, and the note branch is left for a person.

**The first epic needs the same freshness check, by a different route.** It never rebases — under `--pr` step 1 cuts `<noteBranch>` from `origin/<epicBranch>` as it is *now*, and the direct path's `pull --ff-only` fast-forwards onto it — so the ancestry check has nothing to fail on while the proposal can still have been computed against an older brief, and applying it overwrites a concurrent writer's update. Record the sha `read` resolved the brief at; on the first Apply, when `origin/<epicBranch>` no longer names that sha, re-derive exactly as above — fresh `EPIC_CONTEXT`, a fresh live child lookup, Propose re-run, `### Gate` re-entered only on a changed `DIFF` — before applying. **A later epic on the direct path needs that same comparison.** The fetch/ancestry/rebase block above is `--pr`-only because only the note branch can diverge, but a later direct epic's re-checkout can just as easily pick up a brief another writer advanced between this epic's Read and its Apply — and nothing catches it, because the change is already local and the push is therefore not rejected. So every later direct Apply runs the same recorded-sha comparison, live child refresh, Propose re-run and conditional re-gate before applying.

**`BOOTSTRAP: true` first.** Invoke the `notion-dev:epic-doc` skill, operation `record --bootstrap <epic-id>`, passing `REPO_ROOT`, `<epicBranch>` as `<baseRefName>`, and `LOCK_HELD` — exactly as `/notion-dev:next-task` does: it writes the distilled brief, removes the seed plan when there was one, commits the bootstrap, and pushes (under `--pr`, with `--branch <noteBranch>`: commits without pushing). `EPIC-DOC: failed` → this epic fails with that `CAUSE:`; when the cause is a rejected push, the bootstrap commit is the unpushed commit and the loop stops exactly as the third bullet below describes for a note commit — the closeout line names the bootstrap commit's SHA; any other cause continues with the next epic. Otherwise re-derive the proposal against the bootstrapped brief: on the direct path re-run the Read and Propose steps above. Under `--pr` do **not** invoke `read` — the bootstrap commit is local to `<noteBranch>`, while `read` always resolves the brief on `origin/<epicBranch>`, where it is not yet present — so take `EPIC_CONTEXT` from `git show HEAD:<brief path>` (`<brief path>` is the `PATH:` of the bootstrap's `EPIC-DOC:` block), keep the `CHILDREN` already in hand, and re-run the Propose step alone. Either way, re-enter `### Gate` only when the new `DIFF` differs from the accepted one.
...
**The `apply` section ends here, for this epic:** before the next epic's Read and Gate — so no interactive Gate is ever held under the lock and the 60-minute stale rule cannot fire on a human waiting at one — on every path, including a rejected push that stopped the loop, `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/knowledge.py" lock release --run <run id>`.

...
When that check fails: `git -C $REPO_ROOT rebase origin/<epicBranch>`: the note commits are local and unpushed, so nothing published is rewritten and no merge commit is manufactured. A rebase that succeeds means the brief this epic's proposal was computed against has moved, so re-derive before applying — the same re-derivation, for the same reason, that the `BOOTSTRAP: true` path below already performs: take `EPIC_CONTEXT` from `git show HEAD:<brief path>`, keep the `CHILDREN` already in hand, re-run the Propose step alone, and re-enter `### Gate` only when the new `DIFF` differs from the accepted one. Applying the pre-rebase proposal would write a brief derived from content another writer has already landed. A conflicting rebase runs `git -C $REPO_ROOT rebase --abort`, releases the lock, and stops: this epic and every later one are reported `skipped — note branch could not be rebased onto <epicBranch>: <git's message>`, and the note branch is left for a person.

...
**The `apply` section ends here, for this epic:** before the next epic's Read and Gate — so no interactive Gate is ever held under the lock and the 30-minute stale rule cannot fire on a human waiting at one — on every path, including a rejected push that stopped the loop, `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/knowledge.py" lock release --run <run id>`.

...
Line 3 is what makes it *only* the merge. A primary carrying local-only commits satisfies both earlier lines — the branch name is right and the merge commit is an ancestor — while HEAD also holds commits that were never pushed and never reviewed. `git.postMergeHooks` is documented to commit and push, so a hook running there publishes that local work to the base branch as a side effect of a ticket run. Requiring HEAD to equal `origin/<baseRefName>` exactly admits the one state the hook contract promises and rejects divergence rather than reconciling it, which is also why step 4 pulls `--ff-only`.

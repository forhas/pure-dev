...
- marker `"state": "running"` and `heartbeat` younger than 2 hours (compared against `date -u` now — the marker stores UTC) → abort with `held by a live session — <phase> since <heartbeat>`. Interactive mode offers take-over via `AskUserQuestion` (rewrite the marker's `run`, `phase` and `heartbeat` for this run; `state` stays `running`, `worktree` and `branch` are unchanged by construction, then re-read the marker — a race here aborts the same way as the resume branch below, writing nothing else — then continue below); non-interactive never takes over.
- marker `stopped`, a heartbeat older than 2 hours, or no marker at all (a worktree from before markers existed) → resume: rewrite the marker as `running` for this run, then re-read the marker; when it no longer names this run (`run` differs), another session resumed the same worktree in the window — abort with `held by a live session — <phase> since <heartbeat>` exactly as above, writing nothing else — and continue with the resume rules below.

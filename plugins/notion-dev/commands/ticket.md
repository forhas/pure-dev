...
**On a failed, zero-byte, or timed-out dispatch: do not retry, and do not stop.** Record `unexpected:record-unit-not-dispatched` per `notion-dev:issue-log`, then **read `${CLAUDE_PLUGIN_ROOT}/references/record.md` and run it inline yourself**, exactly as `/notion-dev:finalize`'s `MERGED` post-merge recovery path does. The work is idempotent and the fallback is the path that already exists; only this run's context saving is forfeited. Say so plainly in Phase 10's report.

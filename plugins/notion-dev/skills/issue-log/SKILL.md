...
| `findEpics()` returns `[]` | The marker property is **usable** (present and Checkbox-typed); no page is marked yet. Nothing is wrong. Distinct from the `null` return, which **is** logged — see the asymmetry note below. |

...
**This does not extend to `getEpicContext` returning `null` because `epicMarkerProperty` is absent from the live DB.** That cause is not exempt. But the recording is owned by `fetchTicket`, not by `getEpicContext` itself: `/notion-dev:ticket <existing-child-ticket>` can reach `getEpicContext` directly, without ever calling `findEpics` or `createTicket` first, yet it always reaches `getEpicContext` only after its own `fetchTicket(currentTicketId)` call, and `fetchTicket` step 4a reads `epicMarkerProperty`'s live schema state for *every* ticket it fetches — not only pages that turn out to be epics. By the time `getEpicContext` would observe this cause, `missing-property:epicMarkerProperty` has already been recorded per `notion-dev:issue-log`, reusing `findEpics`'s registry row rather than minting a second one. `getEpicContext` does not record it again — one signature, potentially more than one entry-writing site, but never two sites both claiming ownership.

**The same non-exemption applies to `getEpicContext` returning `null` because `epicMarkerProperty` exists but isn't a Checkbox type.** A wrong-typed marker is just as unverifiable as a missing one, and the same direct-reach path (`/notion-dev:ticket <existing-child-ticket>`) applies. `wrong-type:epicMarkerProperty` is recorded the same way — by the caller's preceding `fetchTicket(currentTicketId)` call, per `fetchTicket` step 4a, its sole owner — under the registry row that already exists for it. `getEpicContext` no longer records it under its own steam.

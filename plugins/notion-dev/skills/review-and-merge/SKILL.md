...
**Continuous execution — non-interactive mode is *never hand back*, not only *never ask*.** Do not end your turn between the numbered steps, inside a poll or review round, or after a delegated skill or subagent returns. A message that ends with `Next: <the thing you were about to do>` and then stops is the exact failure this rule names, and the sentence above does not reach it: no question was asked, so nothing was left unanswered — the run simply handed back, and in a non-interactive run nobody is watching to type "continue". Announcing what comes next is fine; announcing it *instead of doing it* is the defect. **This skill is unusually exposed, because it is mostly waiting**: a reviewer round that returns, a settle poll that comes back empty, a green-CI gate that clears, a completeness verifier that reports — each is a natural-looking place to stop, and each is followed by work that only this run can do. Stopping after the loop ends but before the final sweep, or after the sweep but before the gates, leaves a pull request that is reviewed and not merged with nobody watching. The only things that end a non-interactive run are the stop conditions this skill names explicitly — an unusable PR state, a bounded wait that timed out, an API that stayed unavailable, a seat the user prohibited, both review seats failing, a pre-merge check that cannot be satisfied, and everything under "Safety rules" — and the final report.
...
2. **All threads resolved**: re-run the GraphQL thread query, paging through every page, and verify every thread has `isResolved: true`.
   ...
   **Run this query in its own command, never in the same shell invocation as the merge, and treat an empty result as a failed read rather than a pass.** A multi-line GraphQL string does not reliably survive quoting when it shares a batch with other commands: measured in a client, the query produced *no output at all* under exactly that arrangement and the batch went on to merge, so the last gate before an irreversible action passed on a read that never returned a value. The standing rule that **a failed read is never evidence** — stated at the trigger re-read, at the `$SEEN` baseline, at the poll loop, and in the `READ_FAILED` branch of `references/github-api.md` — governs here too, and this is the one place where getting it wrong cannot be undone. So: print a resolved/unresolved count, require that count to have actually been printed, and merge only in a later command. Evidence held from earlier in the run is not a substitute; the gate is the re-read.
...
gh api --method DELETE "repos/<headOwner>/<headRepo>/git/refs/heads/<head-branch-encoded>"   # remote only — local branch and worktree are the caller's
```
...
Unencoded, `feature/foo#bar` is sent as `DELETE /repos/…/git/refs/heads/feature/foo`: `#bar` is read as a URL fragment and dropped, so an unrelated `feature/foo` is deleted instead.

**Delete from the repository that owns the head branch, never from `origin` unconditionally.** On a fork-based PR the head branch lives in the *fork* while `origin` is the base repository, so `git push origin --delete <head-branch>` either fails — leaving the real head branch behind — or, if the base repository happens to carry an unrelated branch of the same name, **deletes that one instead**. `--delete-branch` got this right by resolving the head repository, and dropping the flag must not drop that. In the `ticket` flow the head repository *is* the base repository, since that flow pushes the ticket branch to `origin` itself; `finalize` takes an arbitrary PR number, so it is the entry point that can actually be handed a fork PR. A `403` on a fork you have no write access to is the expected outcome, not a failure: report it and continue; the branch is the contributor's to delete.

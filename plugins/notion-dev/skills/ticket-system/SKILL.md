...
3b. **Set the marker.** Call `mcp__notion__notion-update-page` on `pageId` (step 3a) to set `epicMarkerProperty` to `true`. This is the write that actually makes the page an epic — the Epic select, the body sections, and even having zero children are all incidental to identity. `epicMarkerProperty` is guaranteed present at this point; step 1 already returned `null` otherwise.

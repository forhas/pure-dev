...
| `getEpicContext` | `epicId`, `currentTicketId` | bounded markdown context block, or `null` — epic identity, verbatim `## Overview`, **live** sibling status (via `listEpicChildren`, never the epic body's stale `## Tasks` snapshot) with the sibling whose `id` equals `currentTicketId` marked, and the most recent 3 `## Resolution Log` entries (with a count of any older entries omitted). `null` (no warning — routine) when `epicId` is empty; `null` (record `missing-property:epicMarkerProperty` per `notion-dev:issue-log`) when `epicMarkerProperty` is absent from the live DB instead; `null` (record `wrong-type:epicMarkerProperty` per `notion-dev:issue-log`) when the property exists but isn't a Checkbox type — three distinct causes, none interchangeable. When `currentTicketId` matches no sibling, mark nothing and continue — never an error. The sole owner of what "epic context" means — callers never assemble it themselves or parse `## Resolution Log`. **Background, not requirements**: callers must never treat its content as spec |
...
| `getSelectOptions` | `propertyName` | `[string]` or `null` — lists option names for a Select / Multi-Select / Status property. Returns a bare `null` if the property is absent from the live DB or present but not a selectable type — callers see the same `null` either way and use it as a control-flow signal, unchanged. `propertyName` here is the **live column name** the caller passed in (e.g. callers invoke `getSelectOptions(<phaseProperty>)`, substituting the resolved value, not the key) — not a config key. The adapter, which read the live schema, distinguishes the two causes when recording, but first maps `propertyName` back to whichever config key supplied it and cites that key — annotated inline the same way `createTicket`'s `option-missing:<propertyName>` citation already is (e.g. "here, `epicProperty`") — never the live column name itself: `missing-property:<key>` when absent, `wrong-type:<key>` when present but not Select/Multi-Select/Status. When no config key owns the argument (e.g. a name drawn from `staticProperties`), apply the fallback in `issue-log/SKILL.md`'s signature-grammar section instead of citing the live name. Per `notion-dev:issue-log`. Recording is not a warning — still none on `null`. Read-only. |
...
2. Locate the property named `propertyName` — the **live column name** the caller passed (e.g. `"Phase"`), not a config key.
3. If it's `select`, `multi_select`, or `status`, return the list of option names (`[string]`).
4. If `propertyName` is missing from the live schema entirely, return `null`. Before recording, map `propertyName` back to whichever config key (`phaseProperty`, `epicProperty`, …) supplied this live value, and cite that key — annotated inline the same way `createTicket`'s `option-missing:<propertyName>` citation already is (e.g. "here, `epicProperty`") — never the live column name itself: record `missing-property:<key>` per `notion-dev:issue-log`. When no config key owns this argument (e.g. a name drawn from `staticProperties`), apply the fallback in `issue-log/SKILL.md`'s signature-grammar section rather than citing `propertyName` directly.
5. If `propertyName` exists but is some other type, return `null`. Map it back the same way and record `wrong-type:<key>` per `notion-dev:issue-log`, with the same `staticProperties` fallback when no key owns it.
...
1. Fetch the data source schema; confirm `propertyName` exists and is `select` or `multi_select`. Raise if not. `propertyName` here is the **live column name** the caller passed, not a config key — before recording, map it back to whichever config key supplied it and cite that key, annotated inline the same way `createTicket`'s `option-missing:<propertyName>` citation already is (e.g. "here, `epicProperty`"): record `missing-property:<key>` when absent, or `wrong-type:<key>` when it exists but isn't Select/Multi-Select, per `notion-dev:issue-log`. When no config key owns the argument (e.g. a name drawn from `staticProperties`), apply the fallback in `issue-log/SKILL.md`'s signature-grammar section instead of citing the live name.
...
1. If `epicId` is empty, return `null`. Not a warning — this is the normal case for most tickets, which have no epic. If instead `epicMarkerProperty` is absent from the live DB, also return `null`, but this cause is not routine: record `missing-property:epicMarkerProperty` per `notion-dev:issue-log` before returning. If `epicMarkerProperty` exists on the live DB but is not a Checkbox type, also return `null` — likewise not routine: record `wrong-type:epicMarkerProperty` per `notion-dev:issue-log` before returning, at this same schema check, mirroring the absence case right beside it. A wrong-typed marker leaves epic identity just as unverifiable as a missing one, so it must not fall through to step 2's fetch and get silently swallowed by that step's `false`-collapsing validation. None of these three causes may be conflated — `/notion-dev:ticket <existing-child-ticket>` can reach this operation directly, without ever calling `findEpics` or `createTicket`, so this is the only site that will ever observe the absent- or wrong-type-property cause on that path.

...
| `createTicket` | `{ title, body, type?, epic?, parent?, phase?, step?, assignee? }` | `{ id, url }` — `epic`/`parent`/`phase`/`step` are optional mission metadata; `parent` accepts either a logical ticket id or a Notion page id and is resolved to a page id via `fetchTicket`, same as `setParent`; `assignee` is a resolved Notion user id. Each is absence-tolerant when the corresponding configured property is missing from the live DB |
...
| `findEpics` | — | `[{ id, key, pageId, name, title, url, overview }]` — pages with `epicProperty` set, `parentTaskProperty` empty, **and at least one child** pointing at them via `parentTaskProperty` (the same predicate `/notion-dev:ticket`'s epic guard applies), scoped by `staticProperties` when configured (see "Project scoping guardrail"). `null` when `epicProperty` or `parentTaskProperty` is absent from the live DB (mirrors `getSelectOptions`'s absent-property `null`); `[]` when both properties exist but no epic containers exist yet (this includes any childless Epic-tagged page — see "Epic containers" and the tradeoff noted under `findEpics()` below) — these are different states, not interchangeable |
...
| `getEpicContext` | `epicId`, `currentTicketId` | bounded markdown context block, or `null` — epic identity, verbatim `## Overview`, **live** sibling status (via `listEpicChildren`, never the epic body's stale `## Tasks` snapshot) with the sibling whose `id` equals `currentTicketId` marked, and the most recent 3 `## Resolution Log` entries (with a count of any older entries omitted). `null` (no warning — routine) when `epicId` is empty, or `epicProperty`/`parentTaskProperty` is absent from the live DB. When `currentTicketId` matches no sibling, mark nothing and continue — never an error. The sole owner of what "epic context" means — callers never assemble it themselves or parse `## Resolution Log`. **Background, not requirements**: callers must never treat its content as spec |
...
2. Create the page via `mcp__notion__notion-create-pages`:
   ...
   - **Mission metadata** (absence-tolerant, independent of each other):
     ...
     - If `parent` is provided AND `parentTaskProperty` exists on the live DB, **resolve `parent` to a page id first** — it may arrive as a logical ticket id or already a Notion page id; resolve it exactly as `setParent` step 2 does (via `fetchTicket`) — then set that Relation to a single-element list containing the resolved page id. Applied **after** the page is created and retitled, since it needs the new page's id. `epic` and `parent` are independent: the Epic select makes the grouping visible in DB views and filters, the Parent task relation makes it a container. Callers normally set both.
...
An **epic** is a page in this same database where `parentTaskProperty` is empty, `epicProperty` (Select) is set, and **one or more** other pages point at it via `parentTaskProperty`. Children carry the same `epicProperty` value as the epic page. Epics are identified structurally, not by a dedicated property. The child requirement is load-bearing, not incidental: a page with `epicProperty` set and an empty parent but **no** children is indistinguishable from a legacy Epic-tagged *ticket* on a database upgraded from a version before `parentTaskProperty` existed — `parentTaskProperty` is empty on every pre-upgrade row simply because the column is new, not because those rows are containers. Such a page is not an epic until it actually has a child; see the tradeoff this implies under `findEpics()` below.
...
1. If `epicProperty` or `parentTaskProperty` is absent from the live DB, warn once and return **`null`** — mirrors `getSelectOptions`, which returns `null` for the same absent/unsuitable-property case. Distinct from step 4's `[]`: `null` means epic containers are unavailable on this DB at all; `[]` means the properties exist but no epic containers have been created yet. Callers must not conflate the two.
2. Query the database (or `dataSourceId` when configured) with `mcp__notion__notion-query-data-sources` for **candidate** pages where `epicProperty` is not empty **and** `parentTaskProperty` is empty. When `staticProperties` is configured, add each `[name, expected]` pair as an additional equality filter on this same query — the same scoping the "Project scoping guardrail" applies per-page, applied here at query time so epic discovery never surfaces a foreign project's epic in a shared DB. When `staticProperties` is empty or absent, the query is unchanged.
3. **Filter candidates to actual epics.** For each candidate from step 2, call `listEpicChildren(<candidate's page id>)` and keep the candidate only when it returns **at least one** child. This is exactly the predicate `/notion-dev:ticket`'s epic guard already applies to a single page (see Phase 1.1 there) — unifying discovery on it means "epic" means the same thing in `findEpics`, in the guard, and in the structural definition in "Epic containers" above. A candidate with no children is dropped, not returned as `[]`'s-worth-of-nothing but simply excluded from the result array.
4. For each surviving hit return `{ id, key, pageId, name, title, url, overview }` — `key` is the logical ticket key (`"STO-67"`) for display, same meaning and format as in `fetchTicket` and `listEpicChildren`; `name` is the `epicProperty` Select value, `title` is the page title with the ID prefix stripped, `overview` is the text of its `## Overview` section (empty string when absent). No survivors → `[]`.

**Tradeoff — a childless epic is not rediscoverable.** An epic page created via `createEpic` but left with zero children (e.g. a run interrupted between `createEpic` and the first `createTicket`) fails step 3's filter and will not appear in a later `findEpics()` call. A subsequent mission proposing the same Epic name therefore creates a **second** epic page rather than reusing the orphaned one — a visible, correctable duplicate the user can spot and merge in Notion. This is accepted deliberately, in preference to the alternative of matching on `epicProperty` alone (dropping the children requirement), which would let discovery silently adopt a legacy Epic-tagged *ticket* as a container — turning an implementable ticket into a parent that `/notion-dev:ticket`'s epic guard then permanently refuses to implement. The mission flow itself is unaffected in the normal case: `/notion-dev:create-task` Phase 3.2 holds `EPIC_ID` directly from `createEpic`'s own return value within the same run, so it never needs to rediscover the epic it just created — only a *later*, separate mission run hits this gap, and only when the epic it created was never given a first child.

...
4a. Read the mission/epic-adjacent raw values, each absence-tolerant (missing or unset → that type's empty default, never a warning — these are routine, not exceptional):
   ...
   - `epicMarkerProperty` (if present and Checkbox-typed): the live boolean, or `false` when unset, the property is absent, or it isn't a Checkbox type. **This one property is a named exception to this step's routine-silence rule** (see `issue-log/SKILL.md`'s "What is never logged"): it is the **sole** signal that identifies a page as an epic container, so silently collapsing *why* it read `false` costs a ticket its epic context — the failure this exception exists to close. Before collapsing, distinguish the cause:
     - property **absent** from the live DB → record `missing-property:epicMarkerProperty` per `notion-dev:issue-log`.
     - property **present but not a Checkbox** type → record `wrong-type:epicMarkerProperty` per `notion-dev:issue-log`.
     - property present, Checkbox-typed, simply **unchecked** → routine, record nothing.

     Log at most once per run per cause, same as every other property warning in this file. `metadata.epicMarkerProperty` still returns `false` in all three cases — recording changes nothing about what's returned or any caller's control flow. **`fetchTicket` is the sole owner of this recording.** It is the one operation every path that reads a ticket goes through — `/notion-dev:ticket`, `/notion-dev:finalize`, and any future caller — so detecting the cause here, once, covers every caller without any of them changing. `getEpicContext` no longer records either signature itself; it relies on this step having already run for the current ticket (see its own operation section).
...
1. If `epicId` is empty, return `null`. Not a warning — this is the normal case for most tickets, which have no epic. If instead `epicMarkerProperty` is absent from the live DB, also return `null`. If `epicMarkerProperty` exists on the live DB but is not a Checkbox type, also return `null`. Neither of these two causes is routine, but **neither is recorded here**: `fetchTicket` step 4a is the sole owner of recording `missing-property:epicMarkerProperty` and `wrong-type:epicMarkerProperty` — it already observed and recorded whichever cause applies the moment it read this property for any ticket, including the caller's own `fetchTicket(currentTicketId)` call that resolved `currentTicketId` before this operation was ever invoked. Recording it again here would give one condition two owners, exactly the pattern that kept reopening this bug. A wrong-typed marker leaves epic identity just as unverifiable as a missing one, so neither cause may fall through to step 2's fetch and get silently swallowed by that step's `false`-collapsing validation. `/notion-dev:ticket <existing-child-ticket>` can reach this operation directly, without ever calling `findEpics` or `createTicket` — but it always does so only after its own `fetchTicket(currentTicketId)` call, which is where these two causes are actually observed and recorded, per `fetchTicket` step 4a.

...
1. Fetch the data source schema; confirm `propertyName` exists and is `select` or `multi_select`. Raise if not. Record `missing-property:<propertyName>` when absent, or `wrong-type:<propertyName>` when it exists but isn't Select/Multi-Select (substituting the real property name), per `notion-dev:issue-log`.

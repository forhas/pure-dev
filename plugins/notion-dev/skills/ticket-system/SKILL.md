...
| `setDependencies` | `id`, `[titleOrId, …]` | `void` — renders the ticket's `## Blocked by` body section, **the single owner of that section's format**. Resolves entries that look like titles to tickets within the DB. Writes **no** relation property and reads no live schema, so it has no absence path and records no signature — see the operation section for why direction cannot live in a relation column. Pass-2 in mission creation. |
...
- **Blocking dependencies** — **not a property at all.** There is no `Depends on` column in this plugin's model: a single self-referential Notion relation is symmetric, so it cannot record "A depends on B" without also asserting the reverse. `setDependencies` renders the order into the ticket body as a `## Blocked by` section instead. Nothing here reads or writes a dependency relation, and a `Depends on` column present on a user's DB is left untouched.
...
**This operation writes no relation property, and there is no `dependsOnProperty` config key.** A Notion self-referential relation is symmetric whenever both ends of an edge land in the same column — Notion's own directional self-relation, native Sub-items, needs *two* columns to avoid it. A single column therefore cannot express "A depends on B" without also asserting "B depends on A", so blocking order lives in the body, where it can be stated unambiguously. Writing the relation anyway would add nothing the body doesn't carry while polluting both pages with false reverse edges. Do not reintroduce a relation write here as a "related tickets" convenience: the reverse edges are what made the direction unreadable in the first place.

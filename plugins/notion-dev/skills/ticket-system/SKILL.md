...
| `setDependencies` | `id`, `[titleOrId, …]` | `void` — renders the ticket's `## Blocked by` body section, **the single owner of that section's format**. Resolves entries that look like titles to tickets within the DB. Writes **no** relation property and reads no live schema, so it has no absence path and records no signature — see the operation section for why, which is a limit of the MCP schema surface and **not** a claim that Notion self-relations are symmetric. Pass-2 in mission creation. |
...
- **Blocking dependencies** — **not a property at all.** There is no `Depends on` column in this plugin's model. Not because self-relations are symmetric (they are not — see `setDependencies` for the canonical statement), but because the adapter cannot tell a one-way relation from a two-way one through the MCP schema surface, and a two-way column puts a reverse edge in a companion column it does not control. `setDependencies` renders the order into the ticket body as a `## Blocked by` section instead. Nothing here reads or writes a dependency relation, and a `Depends on` column present on a user's DB is left untouched.
...
The blocker is that **the adapter cannot tell the two forms apart.** The Notion MCP schema surface does not expose a relation's subtype, so a live column named `Depends on` may be either. Bound to a `dual_property` column, every write surfaces a reverse edge in a **companion column the adapter never names, reads, or controls** — and to a caller inspecting the target page's properties, that is indistinguishable from the relation being symmetric. Binding a column whose write behavior cannot be predicted is what this design refuses; the body section states direction explicitly, and no schema detail can change what it means.

Reintroducing the relation would require a reliable way to prove a candidate column is one-way. If the API ever exposes relation subtype, that becomes a real option and this decision should be revisited on its merits — it is a limit of the available surface, not a property of Notion relations.

...
| `setDependencies` | `id`, `[titleOrId, …]` | `void` — renders the ticket's `## Blocked by` body section, **the single owner of that section's format**. Resolves entries that look like titles to tickets within the DB. Writes **no** relation property and reads no live schema, so it has no absence path and records no signature — see the operation section for why: no available signal predicts a relation column's write behavior. **Not** a claim that self-relations are symmetric, and **not** a claim that subtype is undetectable; both were said here before and both were wrong. Pass-2 in mission creation. |
...
| `refreshEpicTasks` | `epicId` | `void` — re-renders the epic's `## Tasks` section from its live children. The single owner of that section's format. No-op when `parentTaskProperty` is **unusable** — absent, or present but not a self-referential Relation — since it has no way to enumerate children. Records `missing-property:parentTaskProperty` or `wrong-type:parentTaskProperty` per `notion-dev:issue-log` for its own path — it returns before `listEpicChildren` is called, so it cannot defer the recording to it |
...
The blocker is that **no available signal predicts how a given column will behave on write.** A previous version of this file said the MCP schema surface does not expose relation subtype at all; that was also wrong, and the correction matters because it is *nearly* right in a way that invites a bad fix.

A subtype indicator does exist: **`propertyUrl`**, which the MCP surface carries on every `dual_property` half and on no `single_property` relation (verified across nine relations and six non-relation controls — non-relations never carry it). So a column named `Depends on` is not opaque; you can tell how it was created.

**But `propertyUrl` records how the column was created, not what a write to it will do**, and those come apart. Deleting a two-way relation's companion leaves the surviving half still carrying its `propertyUrl` while it now behaves exactly one-way — writes to it produce no reverse edge anywhere. Verified directly, and it is not a curiosity: it is the state the reporting client's own database was left in. An orphaned dual half and a live dual half are **identical on this surface and opposite in behavior**, so a guard keyed on `propertyUrl` refuses writes to columns that are in fact perfectly safe, with nothing observable to correct the mistake.

That is why the relation stays unused — not undetectable subtype, but a detectable signal that answers the wrong question. The body section states direction explicitly, and no schema detail can change what it means.

Reintroducing the relation needs a signal that predicts **write behavior**: subtype *plus* live companion state, or a direct behavioral probe. Two things would reopen it, and neither is established — the public REST API exposing an explicit `single_property`/`dual_property` discriminator **together with** a way to see whether the companion still exists (untested here; the only available credential is an `mcp.notion.com` OAuth token, and `api.notion.com` rejects it), or the MCP surface gaining companion-state visibility. Do not reopen this on subtype detection alone: that is the trap this paragraph exists to mark.

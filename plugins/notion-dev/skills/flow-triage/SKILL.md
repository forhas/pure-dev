...
Dispatch **one** read-only `Explore` agent (search breadth: medium), using `${CLAUDE_PLUGIN_ROOT}/references/runtime.md`, role `scout`, with the inherited `RUNTIME_STATE` (initialize it for a standalone invocation). The scout must not edit project files or commit; its owned runtime result artifact and publication are the only write exception. If the host's Explore profile cannot publish, use a read-only-task `general-purpose` worker with that one result-write permission. Wait for and consume its actual report, not its launch acknowledgement. Prompt it with the ticket description (title + body), the current directory as the codebase to probe, and this exact required return format:
...
**Completion before degradation.** Use runtime prepare/attach/wait/consume. Pending notification or an idle display is not an empty report. Only confirmed execution failure, true elapsed timeout with confirmed cancellation, or a consumed but unusable report reaches the retry/degradation path above. Relative agent ages and automatic control probes do not establish failure.

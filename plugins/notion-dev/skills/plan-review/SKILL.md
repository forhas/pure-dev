...
9. An explicit statement that it is **review-only**: it must not edit project files, commit, or push. Its only write exception is the owned runtime result artifact and `publish` call from the runtime protocol.

## Step 2 — Dispatch the reviewer

Dispatch **one** `general-purpose` agent with that prompt using `${CLAUDE_PLUGIN_ROOT}/references/runtime.md`, role `plan`. Read the runtime protocol, initialize or reuse `RUNTIME_STATE`, then prepare/attach/wait/consume the worker. The host may return a launch acknowledgement immediately; wait for and consume the actual completed report before continuing.
...
**Completion before degradation.** Apply the failure/retry rule only after the runtime protocol establishes a terminal failure or an actually completed but unusable report. An idle label or launch acknowledgement is not a terminal result. A missing notification is not zero-byte output. Re-check the durable result before cancelling or replacing a worker, and confirm termination before starting a replacement. Do not run an automatic control probe or infer restarting from relative agent age. A genuinely unavailable independent plan review still degrades as above; it does not authorize bypassing the final independent completeness gate.

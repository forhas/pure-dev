---
name: plan-review
description: This skill should be used when an orchestrating flow needs an independent review of a written implementation plan before execution begins — invoked by notion-dev:ticket on the superpowers build path, or directly when the user asks to "review this plan", "check the plan before implementing", or "is this plan sound". Dispatches a fresh review-only agent that verifies the plan against the actual codebase, triages its findings, revises the plan, and returns a machine-parseable verdict. Superpowers-path only; not for the feature-dev flow, which produces no plan artifact.
...
**A dispatch that never returns is a third shape, and neither rule above covers it.** A failed agent errors and a zero-byte agent goes idle — both *end*, which is what lets a rule inspect what came back. This one does not end: the agent stays in `running` state and never emits a final block, so a check written against a result that arrived can never fire. The tell is `ListAgents` reporting the same agent as "started <1m ago" on repeated checks — it is restarting from the beginning rather than making progress — and the nudge above does not reach it, because a nudge is answered by an agent that is waiting, not by one that is looping. **Bound the wait** the way `../review-and-merge/SKILL.md`'s checks gate already bounds its own (~15 minutes), then stop the dispatch and treat it as a failure per **Degradation** above; an unbounded wait is the defect, because the seat is not filling and nothing will say so. Measured in a client: five dispatches across two seats in one run, none delivered, two of them stopped only after 75 and 50 minutes. In that same run an `Explore` scout on the same host delivered in full — so a sibling agent that works is not evidence that this one will, and must never be read as one.

**Tell "this dispatch is broken" from "dispatch is broken on this host" with a control probe.** Both zero-byte and never-returns shapes invite the reading that the prompt was too long, the rubric too complex, or the task too hard — and each of those invites a retry that costs another dispatch to disprove. One cheap probe settles it: dispatch a `general-purpose` agent whose entire prompt is *"Reply with exactly the word PONG and nothing else. Do not use any tools."* A probe that also comes back empty rules out prompt size, rubric complexity, tool use and task difficulty in a single dispatch. Measured in a client, where exactly that probe returned zero bytes and made the failure diagnosable as the harness.

**A seat lost to the dispatch mechanism predicts the merge-gate seats, and that is worth saying — but it is not a reason to stop the build.** `../review-and-merge/SKILL.md`'s local review fallback and its completeness verifier are dispatched the same way this seat is, so when a control probe shows the mechanism itself is down, those seats are likely to be lost too and the work may be built but not mergeable. Say that in the degraded report so the caller can decide before spending the build. Do **not** convert it into a stop: measured in a client, one run lost this seat and the completeness verifier to exactly this condition while the **configured** code reviewer — not a subagent — completed four rounds normally and found seven real defects, so the pull request was reviewed and merged. Degraded still proceeds; what changes is that the caller is told what else is likely gone.

## Step 3 — Triage the findings

Apply the `superpowers:receiving-code-review` skill to the findings: agree, partially agree, or disagree with each one, with reasoning. Never apply a finding blindly, and never perform agreement you do not hold.
...
- **declined** — you disagree, with a stated reason. **A declined finding is resolved and does not block.** (Same rule `../review-and-merge/SKILL.md` already applies to code review.)

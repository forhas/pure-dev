...
3. Otherwise `git commit --only -m "docs(epic): note <KEY>-<n> — <short fact>" -- <brief path> <knowledge.dir>/index.md` (`--only`, for the same reason `record` gives: an exempt setup file may already be staged and must not ride along). Push `git push origin <baseRefName>` — skipped under `--branch`, where the caller pushes once.

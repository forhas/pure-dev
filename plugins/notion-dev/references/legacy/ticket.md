...
**Continuous execution — non-interactive mode is *never hand back*, not only *never ask*.** Do not end your turn between phases, between steps inside a phase, or after a delegated skill or subagent returns. A message that ends with `Next: <the thing you were about to do>` and then stops is the exact failure this rule names, and the self-answer rule above does not reach it: no question was asked, so nothing was left unanswered — the run simply handed back, and in a non-interactive run nobody is watching to type "continue". Announcing what comes next is fine; announcing it *instead of doing it* is the defect. The only things that end a non-interactive run are the stop conditions this command names explicitly — every one of them, wherever it appears — and Phase 10's report. Among them: the preconditions gate, Phase 1's two hard aborts — **the epic guard and the `held elsewhere` ownership check, both of which non-interactive mode must obey** — 1.3's under-spec and blocked-on-external stops, `PLAN-REVIEW: blocked` (4.2b), `retry-exhausted:verify` (5.1), and "Failure and stop conditions". **This list is illustrative, never exhaustive** — a stop this rule forgot to name is still a stop, and reading the list as closed would turn a rule against handing back into a licence to continue past a hard abort.

**This rule is enforced by a `Stop` hook, because as prose alone it did not hold.** `hooks/stop-guard.sh` blocks the stop while this project has a live `--non-interactive` run marker, and names the phase to resume at. **Every phase writes such a marker**: Phase 2.1's `<KEY>-<id>.json` from Phase 2 onwards, and the preconditions gate's `preflight-<session>-<invocation>.json` for the whole of Phase 1, which cannot use the other name because the ticket id is not resolved until 1.1 returns (issue #57). It exists because 0.28.1 shipped the rule above as prose and a client run on 0.28.1 stopped at the end of Phase 7 anyway — then, asked why, quoted the rule it had just broken. Ending a turn is the **absence** of an action, so no instruction can gate it; only the harness can. The prose is still load-bearing: it is what tells you what to do when the guard sends you back. But do not treat this paragraph as the mechanism, and do not assume the guard will catch every case — it is bounded on purpose (a stale marker, or a fourth stop in one session, and it lets go), and `/notion-dev:finalize` writes no run marker, so the guard does not cover it at all. Measured in two clients on the same day: two runs stopped mid-`Phase 4`, one after 4.2(a) and one after 4.2(b), each with a `Next:` line and no question asked.
...
- **Write the preflight run marker — first, before any probe below can abort.** Non-interactive mode only; in interactive mode write nothing. `hooks/stop-guard.sh` blocks a stop only while a run marker exists, and Phase 2.1's `<KEY>-<id>.json` cannot be written here: the numeric `<id>` does not exist until 1.1's `fetchTicket` returns, and the argument may be a page id, a dashed UUID, a URL or a logical key. So this window — the probes below, 1.1's fetch and knowledge retrieval, 1.2's worktree and claim protocol, 1.3's clarification gate — is covered by a second marker shape, keyed by the one identifier that *is* known here: the harness session id. Write it to `$REPO_ROOT/.claude/notion-dev/runs/preflight-<session>-<invocation>.json`, where `<session>` is `$NOTION_DEV_SESSION_ID` with every character outside `A-Za-z0-9._-` replaced by `_`, and `<invocation>` is a token generated **once, here** in 2.1's form — `$(date -u +%Y%m%dT%H%M%SZ)` plus 4 hex. **Both halves are load-bearing and neither substitutes for the other.** The session half is what the guard's counter is scoped by; the invocation half is what stops a *second* `/notion-dev:ticket` in the same session inheriting the first one's spent block counter. Without it, a run that exhausted the cap in Phase 1 and then ended its turn leaves a `running` marker and a counter reading `MAX_BLOCKS` on disk; the next invocation overwrites that same session-keyed marker, the counter survives pruning because its marker exists and is `running`, and **every Phase 1 stop of the new run is allowed immediately** — the enforcement present and silently off, which is the failure mode this whole mechanism exists to remove. `/notion-dev:next-task` runs tickets back-to-back in one session, so this is the ordinary path, not the exotic one. **Create the self-ignoring directory first, and spell all three paths against `$REPO_ROOT`:** `mkdir -p "$REPO_ROOT/.claude/notion-dev"`, then `[ -f "$REPO_ROOT/.claude/notion-dev/.gitignore" ] || printf '*\n' > "$REPO_ROOT/.claude/notion-dev/.gitignore"` — the two commands from `skills/flow-triage/references/ledger.md`, **which are written relative there and must not be copied relative here** — then `mkdir -p "$REPO_ROOT/.claude/notion-dev/runs"`, in that order. The relative form is wrong on the no-arg resume path, which starts inside the ticket worktree: the ignore file would land in the worktree while the marker goes to the absolute `$REPO_ROOT` path, so the primary checkout gains an untracked marker and fails the cleanliness gate anyway — the same abort, one directory over, on the one path the relative form looks harmless from. This marker is now the earliest thing this command writes, earlier than the ledger, the criteria file and the rescued `PLAN.md`, so on a repository's first ticket run nothing has created `.claude/notion-dev/.gitignore` yet. Written without it the marker is an untracked path, the clean-tree precondition four bullets down sees dirt that is not on its exhaustive exempt list, and **the run aborts on a file it created itself** — on the first run in every newly initialised client, which is exactly where it would be read as the plugin not working:

  ```json
  { "run": "<the argument as supplied, with any \" or \\ removed — `resume` when there was none>",
    "session": "<invocation>", "phase": "Phase 1 — preconditions", "heartbeat": "<date -u +%FT%TZ>",
...
- **Retiring the preflight marker is not optional. Two moments retire it, whichever comes first.** (An interactive run wrote no marker, so both are no-ops there; every instruction below is safe to run either way — `rm -f` on an absent file, and nothing to rewrite.) **(1) Handover** — `rm -f` the file the instant a `<KEY>-<id>.json` marker naming this session is live: 2.1's write on the fresh path, 1.2's resume rewrite and 1.2's take-over rewrite on the others. It is a placeholder for that marker and must never outlive one. **(2) A stop** — on **any** path that ends the run before 2.1's marker exists, rewrite the preflight marker with `"state": "stopped"` and `cause` set to the one-clause reason before stopping, exactly as "Failure and stop conditions" does for the run marker; that section's rule is this rule, applied to whichever marker exists at the time. **Skip it and the guard refuses the stop the command itself just ordered** — a documented hard abort becomes a session told three times to carry on with a run that has already decided not to, which is strictly worse than the gap this marker closes. Every stop before 2.1 is one of these and each says so where it appears: every precondition abort in this list, 1.1's missing-`idProperty` stop, 1.1's epic guard, 1.1's `held elsewhere` ownership check, 1.2's `held by a live session` aborts and its lost-claim aborts, 1.3's under-spec stop, 1.3's blocked-on-external stop, and 2.1's `claimed-elsewhere`. **That enumeration is a checklist, not a boundary** — a stop it forgot is still a stop and still takes (2); the guard's `MAX_BLOCKS` cap is what keeps a forgotten one costing three refusals instead of a wedged session, and a bound is not a licence to rely on it.
- **Superpowers and feature-dev (required live).** Follow `${CLAUDE_PLUGIN_ROOT}/references/dependencies.md` in `ticket` mode before side effects. Confirm `superpowers:writing-plans`, `superpowers:subagent-driven-development`, `superpowers:receiving-code-review`, and `feature-dev:feature-dev` in this host's available skills. `dependencies.superpowers` and `dependencies.featureDev` are cached hints only: report discrepancies, never gate on their booleans. Missing skills stop with the diagnosed enable/reload/install remedy; never reinstall over a disabled local setting.
...
Derive the **numeric `<id>`** used for all naming below from `metadata.idProperty value` in the returned ticket. Rewrite the preflight marker now: `run` becomes `<KEY>-<id>` and `phase` becomes `Phase 1.1`, per the preconditions rule — this is the first point at which the guard's block message can name the ticket rather than the argument. If the resolved page has no `idProperty` value, stop and tell the user the ticket DB needs an ID property — branch and worktree naming depend on it; **write `"state": "stopped"` with that cause into the preflight marker before stopping** (preconditions rule (2)).
...
Hard abort in both interactive and non-interactive mode. It runs before Phase 2, so no worktree, branch, status change, or ledger line is created — and therefore before the `<KEY>-<id>` run marker exists, so the marker this stop must retire is the preflight one: **write `"state": "stopped"` with the cause into `$REPO_ROOT/.claude/notion-dev/runs/preflight-<session>-<invocation>.json` before stopping** (preconditions rule (2)). Non-interactive mode obeys this abort, and the stop guard is what would otherwise refuse it three times over.
...
Rewrite the preflight marker's `phase` to `Phase 1.2` on entry (preconditions rule). **Every abort in this section stops before 2.1's marker exists, so every one of them writes `"state": "stopped"` with its cause into the preflight marker first** — the `held by a live session` aborts below, both lost-claim aborts, and the two re-read aborts. This is the section with the most stop paths in Phase 1 and the one where a missed retirement costs most: the run has just been told another session owns the ticket, and the guard would send it back into that ticket three times.
...
- marker `"state": "running"` and `heartbeat` younger than 2 hours (compared against `date -u` now — the marker stores UTC) → abort with `held by a live session — <phase> since <heartbeat>`. Interactive mode offers take-over via `AskUserQuestion` (claim it exactly as the resume branch below does — the same `mkdir` claim, the same fresh `session`, the same re-read — rewriting `session`, `phase`, `heartbeat`, **`claude_session` and `non_interactive`**; `state` stays `running`, `worktree` and `branch` are unchanged by construction; `rm -f` the preflight marker once that rewrite has landed, per preconditions rule (1) — 2.1 never runs on a take-over, so this branch is the only place the handover can happen; the last two are as load-bearing here as on the resume branch and fail in opposite directions — a stale `claude_session` leaves the taking-over session unguarded, while a `non_interactive: true` inherited from the run being taken over would have the guard refuse the stops of an interactive session that is legitimately pausing to ask; a lost claim aborts there, writing nothing else, and nothing below runs); non-interactive never takes over.
- marker `stopped`, a heartbeat older than 2 hours, or no marker at all (a worktree from before markers existed) → resume, and claim the resume atomically before writing anything: `mkdir "$REPO_ROOT/.claude/notion-dev/runs/<KEY>-<id>.claim"` — the same atomic primitive §4's primary lock is built on. Create the parent `$REPO_ROOT/.claude/notion-dev/runs/` with `mkdir -p` first: it does not exist on a fresh install, nothing else creates it until the marker is written, and the claim would otherwise fail with `ENOENT` rather than because someone holds it. Never `mkdir -p` the claim directory itself — failing when it already exists is its entire value. A plain rewrite-and-read cannot stand in for it: two sessions that each write and then read their own write both see themselves and both proceed into one worktree.
  ...
  - `mkdir` succeeds → **re-read the marker before rewriting it**, because the decision to resume was made from a read taken before the claim: another session may have held the claim in between, published a fresh marker and released it, and overwriting that now would put two sessions in one worktree while its own post-write re-read still succeeded. When the re-read marker is `running` with a heartbeat younger than 2 hours, or names a different `session` than the one this decision was made from, abort with `held by a live session — <phase> since <heartbeat>`, `rmdir` the claim, and run none of the resume rules below. Otherwise rewrite the marker as `running` with a fresh `session` **and this invocation's `non_interactive` and `claude_session`** (2.1 defines both; the resuming run's mode and session are the ones that count, never the ones recorded by the run being resumed), re-read it, then `rmdir` the claim directory and `rm -f` the preflight marker — preconditions rule (1), and this branch needs it stated because a resume jumps past Phase 2.1, which is where the handover otherwise happens. **`claude_session` is not optional here and its omission is silent**: the guard matches the marker against the stopping session's id, so a marker left carrying the previous owner's id — or none, on a marker written before 0.29.0 — matches nobody, and stop protection is off for the whole resumed run with nothing to show for it. Only when the re-read marker still names this `session` do the resume rules below run; when it no longer names this run (`session` differs), abort exactly as above, writing nothing else.

**A marker whose protected re-read still read `running` is a mid-phase end, and is recorded.** The values this tests are the ones the `mkdir`-succeeds bullet above observed **before** it rewrote the marker: capture `state`, `phase` and `heartbeat` from that re-read, and test the captured values here. **Testing the marker as it stands at this line instead reports nothing while appearing to match everything** — that bullet has already rewritten it to `running` with this run's own fresh `session` and `heartbeat`, so a literal state test is true on every resume including a clean `stopped` one, and the stale-heartbeat qualifier that would narrow it can never be true at all. Every stop path this command has writes `"state": "stopped"` with a `cause` (see "Failure and stop conditions"), so a **captured** `running` whose captured `heartbeat` is older than the 2-hour threshold means the previous invocation reached no stop path at all — it ended its turn mid-phase, or its session died. Record `unexpected:run-ended-mid-phase` per `notion-dev:issue-log`, carrying the **captured** `phase` as `Where` — that field is the whole diagnostic value, because it names where the run let go, and the rewrite has already overwritten it. A **missing** marker is not this condition (a worktree from before markers existed), and neither is a captured `stopped`; only a captured `running` is. This resume is the **only** place the condition is observable at all: a run cannot notice its own turn ending, so without this entry a non-interactive run that hands back mid-phase leaves nothing in the log and is visible only to whoever happens to look at the screen.
...
Rewrite the preflight marker's `phase` to `Phase 1.3` on entry (preconditions rule). **Both stops below write `"state": "stopped"` with their cause into that marker before stopping** — they are the last two stops that happen before 2.1's marker exists.
...
- **If the requirement is clear but not satisfiable *now*** — any mandatory prerequisite gates on an event outside this run that has not happened yet (a customer-operated deploy, a human sign-off, an observation window still elapsing): **stop, and report it as blocked on a named external precondition — not as under-spec.** A watch ticket can be perfectly well-formed and still unbuildable, and the question this gate asks (*do I understand it well enough to implement without guessing?*) answers **yes** for one, so nothing above catches it. The distinction decides what the user does next: elaboration is the remedy for the bullet above and is useless here, so sending a well-specified ticket to `/notion-dev:create-task` costs a round trip and improves nothing. Name the precondition and what would clear it, exactly as a `blocked` disposition does everywhere else in this plugin — `blocked` is for a **named external cause**, and this is one. Measured twice in one client, on a ticket and its direct successor.
...
**The claim is the worktree.** `git worktree add … -b ticket/<project.key>-<id>-<slug>` is atomic on one machine: when it fails because the branch already exists and 1.2 found no worktree of ours, another session claimed the ticket in the window between 1.2 and here. End the run with the outcome `claimed-elsewhere`: print one line, `OUTCOME: claimed-elsewhere — [<key>] branch ticket/<project.key>-<id>-<slug> already exists; another session holds it`, write `"state": "stopped"` with that cause into the preflight marker (preconditions rule (2) — this stop happens before the run marker below is written, so the preflight one is still what the guard is holding), and stop **before** any status change, ledger line, or brief write. Nothing to clean but the `.claim` directory this path took, nothing to report beyond that line; `/notion-dev:next-task` reads this outcome and picks another ticket; it is a run outcome, recorded nowhere but this line and the caller's decision log — never an issue-log entry. A `git worktree add` failure for any other reason (path exists, fetch failed) is the ordinary stop.
...
{ "run": "<KEY>-<id>", "session": "<KEY>-<id>-<YYYYMMDDTHHMMSSZ>-<4 hex>", "worktree": "<worktree-path>", "branch": "ticket/<project.key>-<id>-<slug>",
  "phase": "Phase 2", "heartbeat": "<date -u +%FT%TZ>", "state": "running", "non_interactive": <true|false>,
  "claude_session": "<$NOTION_DEV_SESSION_ID>", "cause": null }
```

**Then `rm -f "$REPO_ROOT/.claude/notion-dev/runs/preflight-<session>-<invocation>.json"` — the handover, and it belongs here rather than anywhere later.** The preconditions gate wrote that marker to cover Phase 1, where this one could not exist yet; from this line on it can, and two live markers for one session are one more than the guard needs. Nothing breaks while both exist — they carry the same `claude_session`, so the guard blocks either way — but the preflight marker's `phase` stops advancing at 1.3, so a block that matched it would name a phase this run left behind. Delete it immediately after the write above, not after the status update and not at the end of the phase: the interval between the two is the whole window in which the wrong phase can be reported.

**`non_interactive` records this invocation's mode, and it is not decoration** — `hooks/stop-guard.sh` reads it, and it is the field that decides whether the guard may block a stop at all. Write the mode of *this* invocation, not the one the marker was created with: a run resumed interactively after a non-interactive one is interactive now. An interactive run ends its turn to ask, which is correct, so the guard blocks only on an explicit `true`; absent or `false` it never fires. A marker written by a version before 0.29.0 has no such field and is therefore treated as interactive — the guard degrades to doing nothing rather than to blocking the wrong run.

**`claude_session` is the coding harness's own session id** — read from **`$NOTION_DEV_SESSION_ID`**, and **not** the `session` field above, which is this plugin's run token and means something else entirely. `$NOTION_DEV_SESSION_ID` is published into this session's environment by this plugin's own `SessionStart` hook (`hooks/session-env.sh`), which writes it through `$CLAUDE_ENV_FILE` — the documented mechanism for persisting a variable into a session. **Do not read an ambient `CLAUDE_CODE_*` variable instead.** The id a run must record is the one the harness passes to a hook on stdin, and that is only reliably available inside a hook; a command reading some ambient variable may find it unset, write an empty owner, and leave the guard skipping that marker for the whole run — the enforcement present, enabled and inert, with nothing on screen to say so. When `$NOTION_DEV_SESSION_ID` is itself empty — the hook did not run, or the harness is not Claude Code — record the empty string and say so in the report: an unattributable marker blocks nobody, which is the right direction to fail, but it does mean this run has no stop protection. It exists so the guard blocks **only the session that owns this run**. One checkout can hold two ticket runs at once — parallel tickets on one machine are supported — and it can hold an interactive session alongside a non-interactive one; without this field the guard would refuse the stop of whichever session happened to stop first and tell it to continue another run's ticket from another run's phase, which is precisely the wedge the guard exists to avoid. Write the empty string when the variable is unset: an unattributable marker blocks nobody, which is the right failure direction. Rewrite it on resume with the resuming session's id, for the same reason `non_interactive` is rewritten there.

`run` names the ticket and is identical for every session of it, so it can never tell two of them apart; `session` is generated once per invocation — #44's token form, `$(date -u +%Y%m%dT%H%M%SZ)` plus 4 hex — and is the field 1.2's claim compares. Marker discipline for the rest of the run: rewrite `phase` and `heartbeat` at every phase boundary and every review round (Phase 7 says where), and after every build task and verify iteration; set `"state": "stopped"` with `cause` on the failure path, and **delete the file in Phase 10, last, immediately before printing the summary — never in Phase 9 cleanup**. The marker is what `hooks/stop-guard.sh` matches on, so deleting it with the worktree would leave branch cleanup, the post-merge hooks, the record unit's return and the whole of Phase 10 unguarded; this sentence said Phase 9 step 1 until 0.29.0 and is the operational instruction a run actually follows, so leaving it stale would have reinstated that gap on its own. A JSON file this flow writes directly; no script. The marker is what tells a second session — and `/notion-dev:next-task` — whether this claim is live (1.2's rules). A heartbeat says *this run reached that boundary*, never *this run is alive right now*: it is written between units of work and never inside one, because a flow blocked in a dispatched build task, a running verify `cmd`, or a reviewer round cannot write anything until that unit returns. Those boundaries are the resolution of the signal, so 1.2's 2-hour threshold bounds the longest single unit rather than the run — it has to stay longer than any one build task, verify `cmd`, or reviewer round, which is what makes a stale heartbeat mean abandoned instead of busy.
...
**Leave the worktree: `cd $REPO_ROOT`.** `${CLAUDE_PLUGIN_ROOT}/references/legacy/record.md`'s cleanup removes the worktree, and the orchestrator is normally sitting inside it.
...
**Dispatch one `general-purpose` agent using the runtime protocol**, role `record`, with a self-contained prompt carrying: `RUNTIME_STATE`, the prepared worker ID and publication contract, the ticket id, the ticket body, PR number and URL, `$REPO_ROOT`, the worktree path, the branch name, `baseRefName`, `<merge-commit>` (the SHA `notion-dev:review-and-merge` returned), the merge strategy it used (`squash` / `merge` / `rebase`), `RUN_START`, `REVIEW_REPORT`, `COMPLETENESS_REPORT`, `PLAN_REVIEW_REPORT`, `COMPLETION_CLOSEOUT`, `KNOWLEDGE_CONTEXT`, `DECISIONS` (this run's non-interactive decisions), `DRAFT_REPORT`, the `CRITERIA_FILE` path, the run id, **the `session` token** — the run marker's `session` field, `<KEY>-<id>-<YYYYMMDDTHHMMSSZ>-<4 hex>`, which the unit keys its replay guards and hook checkpoints by and **must be given rather than read**: the marker does survive the unit now (it is deleted in Phase 10, not in cleanup — see `references/legacy/record.md` step 1), but a unit that recovered `session` by reading the marker would silently pick up a *later* run's token on a replay, which is the bug keying by `<run id>` already causes — `FILING_DECISIONS`, `LOCK_HELD: true`, **`DISPATCHED: true`** — the key that tells `references/legacy/record.md` it is running inside an agent rather than in this main loop, so it passes `NO_PROXY: true` down to `epic-update` and no follow-up filing dispatches a proxy respondent from in there; the inline recovery path below omits it and keeps the proxy, because there the main loop is the review author `create-task`'s proxy-respondent section is about — `--non-interactive` when this run has it, and the instruction to read `${CLAUDE_PLUGIN_ROOT}/references/legacy/record.md` and follow it exactly. Record its `RECORD:` block as `RECORD_REPORT`.
...
**Bound the wait at ~15 minutes** using the runtime's actual elapsed deadline, not an agent-list age or guessed minutes. Configure the record worker with `--timeout 900`; when `git.postMergeHooks` is configured use `--timeout 2700` (~45 minutes), preserving the existing margin before the primary lock's 60-minute stale threshold. Prepare immediately before dispatch. Pending or quiet delivery is not a failed record. Consume an available durable result first. For true timeout, request cancellation through the runtime, confirm host termination, and only then permit inline recovery; preserve the held-lock stop rule below if termination is unknown.
...
Record runtime stage `complete` only after recording and workspace closeout finish, then include the `runtime.py summary` evidence path and a compact measured timing/worker/verification summary. On a stop use stage `stopped`, not `complete`. If raw host logs are available, collect `telemetry.py` usage with its explicit cutoff; unavailable token counters remain unknown.
...
**Delete the run marker — last, immediately before printing the summary below.** `rm -f "$REPO_ROOT/.claude/notion-dev/runs/<KEY>-<id>.json"`, plus the guard's counters for this run (`rm -f "$REPO_ROOT/.claude/notion-dev/runs/.stop-guard-<KEY>-<id>--"*`). **Not in Phase 9 cleanup, which is where it used to happen**: the marker is what the `Stop` guard matches on, so removing it with the worktree left every later step unguarded — branch cleanup, the base checkout and pull, the post-merge hooks, the record unit's return, and this entire phase including the closeout pass. Deleting it here leaves exactly one unguarded moment, the printing of a summary that is itself the end of the run. On the failure paths the marker is rewritten `stopped` instead and never deleted, so a person can still see where the run got to.

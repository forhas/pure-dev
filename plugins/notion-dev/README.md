...
- **First use.** With no brief yet, `/notion-dev:next-task` creates and commits one before it starts — distilled from an existing hand-written plan named after the epic (`docs/*<KEY>-<n>*.md`), which it then `git rm`s so two files cannot drift against each other (the last base commit that still holds it is cited on the brief's header line, so `git show` recovers the detail); or from the Notion epic when there is none. A `/notion-dev:ticket` run started directly on a child of an epic with no brief works from an in-memory version and creates the file at its own resolution.
...
│   ├── epic-doc/             # per-epic markdown brief: read at ticket start, rewritten at resolution, noted by new-info (used by ticket, finalize, next-task, new-info)

...
- **`iwe` ≥ 0.19 on `PATH`** — **required**. `notion-dev:knowledge` uses it for budgeted search-plus-graph retrieval and dedupe over the knowledge bundle. Two install routes: `cargo install iwe --root ~/.local` (works wherever Rust does; required on hosts whose GLIBC is older than 2.39, which includes Ubuntu 22.04 under WSL); or `brew install iwe` / `npm i -g @iwe-org/iwe` where the prebuilt binary runs.
- **`python3`** — **required** for `scripts/knowledge.py`, the bundle's shape validator, migrator, and touched-concept finder. Standard library only; no `pip` step.
...
- **One writer, one commit path.** Every resolution — `/notion-dev:ticket` and `/notion-dev:finalize` alike — captures via the post-merge hook, rewriting the brief and writing any new concepts a merge evidenced, then commits straight to the base branch (`docs(knowledge): capture <KEY>-<n>`) under the same git assertions the post-merge hooks use. `/notion-dev:knowledge capture <ticket-id> <merge-sha>` re-runs the same write by hand when the hook was skipped or failed.

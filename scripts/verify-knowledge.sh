...
if [ -f "$ED" ]; then
  ...
  assert_count "epic-doc: \`<knowledge.dir>/index.md\` is written and committed by every operation that writes the brief (cited 7 times: twice in the catalog rule, once in record's pathspec, once in the bootstrap's, three times in note-apply — its no-op test, add and commit — tune this count in the same commit that changes the section, per CLAUDE.md)" \
    "$ED" 1 "$L" '<knowledge\.dir>/index\.md' 7

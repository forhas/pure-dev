...
# the second is wired:
#
#   GRAMMAR (here, every platform, every push) -- a CHEAP EARLY FILTER, and its reach is
#   narrower than it looks. Measured against CPython 3.12's ast: `feature_version=(3,8)`
#   rejects pattern matching and parenthesized context managers, and ACCEPTS `except*`
#   (3.11), `X | Y` annotations (3.10) and builtin generics like `list[str]` (3.9). So this
#   is a smoke test, not the authority, and it must not be described as one.
#

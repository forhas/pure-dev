...
assert_present "apply: the fetch runs on every epic, so the first epic's recorded-sha comparison is meaningful" "$NI" "$A0" "$A1" 'on \*\*every\*\* epic.*`git -C \$REPO_ROOT fetch origin <epicBranch>`. Not only on a later one'

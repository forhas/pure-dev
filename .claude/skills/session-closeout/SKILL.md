...
5. *(completion, then workspace)* **Issues this session filed**, plus every item in a review
   ...
   **A review report's `BLOCKED` list is not part of this source.** Those items are already in
   ...
   requires — decided by the triage that produced them (`quick-dev:review-and-merge` `## 2`).
   They pass straight through to the report's `blocked:` lines. Do **not** demand a ticket URL
   or a `Deferred:` trailer for one: that would convert an externally impossible item into a
   queue entry nobody can close, which is precisely what giving it its own disposition was for.
   Re-judge one only if its cause has since cleared, in which case it was never `blocked` and
   goes back to triage.

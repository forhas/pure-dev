...
**A failed trigger has an unknown outcome — never blind-retry it.** `gh` exiting non-zero does **not** mean the request never reached GitHub: a transport failure (DNS, TLS, `error connecting to api.github.com`) can lose the *response* after the comment was already posted. Retrying blind then double-triggers, so two reviews race on one poll baseline and the round counter is wrong for the rest of the run. **Capture an attempt baseline immediately before every trigger post — including the first one of the run.** The recovery read needs something definite to compare against, and "newer than the round's previous trigger" is undefined on the first post; a PR can already carry an older `@codex review` comment (a previous run, or a human), so with no baseline the recovery read can adopt that stale comment as proof the new post landed — skipping the retry *and* polling from a stale timestamp, which makes an old review look like this round's response. Record the current UTC time or the newest existing `@codex review` comment id before each post.

On a non-zero exit, re-read the state the trigger would have changed and let that decide — never the exit code alone: `gh api --paginate repos/{owner}/{repo}/issues/<pr>/comments` and look for an `@codex review` comment newer than **this attempt's baseline**. Present → the post succeeded; adopt its `created_at` as the trigger timestamp and continue. Absent → nothing landed; retry the post, at most 3 times with a short backoff (~10s), re-checking state before each retry. Still failing with no state change → stop and report; a transport fault is never an unavailability signal, so do not fall through to the local loop on one.

**The state re-read must itself succeed before it decides anything.** A read that errors, times out, or returns empty is not evidence of either outcome — never compare a failed read's empty result against the baseline and conclude from the difference that the state did (or did not) change. That turns one network fault into a false verdict: a false "it landed" silently skips the round, a false "it didn't" double-triggers. Retry the **read** until it returns a definite answer, then decide.
...
- **Not-configured / error / refusal** — a message from the Codex app itself (author login exactly `chatgpt-codex-connector[bot]`, matching the response filter above) or explicitly about Codex (e.g. a workflow notice that Codex is disabled or not installed) that is *exclusively* an inability-to-review notice: it carries **no** `Reviewed commit` marker and **no** findings. Two guards matter here: the exclusivity guard — a normal review that merely mentions an error while still carrying findings or a reviewed-commit marker is a normal round, not an unavailability signal — and the author guard — another review/CI app's failure notice is never a Codex signal. `reason=not-configured` when the message says Codex is disabled / not set up / no app installed; otherwise `reason=error`.
- **Silence** — no new Codex response within **~10 minutes (20 polls)** of a trigger. **The window elapsing is not proof the request is dead** — reviewer latency varies widely across rounds of the same run, and re-triggering a merely-slow request queues a second review and produces a duplicate round. Before re-triggering, re-read reviews and issue comments with a **definite** read (a failed read is not silence); if still nothing, keep polling in further ~10-minute extensions to a bounded total of ~30 minutes. Only then **re-trigger once** (re-post `@codex review`, re-poll one more ~10-minute window — this re-trigger does not increment the round counter). If a review lands on the retry, continue normally. If the retry window is also silent: `reason=silent`. If a duplicate round happens anyway, triage **every** review that arrived — do not let the newer one mask the older one's findings.
